...
// framed by these sentinel markers; execution-controller splits the pod log on them
// to extract the structured JSON it uploads as run_results_uri. These constants are
// the single Go source of truth — the cross-repo guard in markers_test.go pins them
// to the literal wire strings so the two sides cannot drift silently.
...
// result_block() (its SCHEMA_VERSION constant). execution-controller checks a
