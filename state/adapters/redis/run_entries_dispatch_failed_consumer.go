...
// RunEntriesDispatchFailedConsumer consumes run.entries.dispatch_failed:v1 events.
// On a fresh failure it finalizes scheduler_tracker as failed and emits run.finalized:v1.
// Counterpart of RunEntriesDispatchedConsumer.
type RunEntriesDispatchFailedConsumer struct {
	client        *goredis.Client
	streamName    string
	consumerGroup string
	consumerName  string
	handler       *statehandlers.RunEntriesDispatchFailedHandler
	logger        *slog.Logger
	stopCh        chan struct{}
}

// NewRunEntriesDispatchFailedConsumer creates and initialises the consumer group.
func NewRunEntriesDispatchFailedConsumer(
	client *goredis.Client,
	streamName string,
	db *sqlx.DB,
	...
	logger *slog.Logger,
) (*RunEntriesDispatchFailedConsumer, error) {
	handler := statehandlers.NewRunEntriesDispatchFailedHandler(db, schedulerRepo, outboxRepo, logger)
	c := &RunEntriesDispatchFailedConsumer{
		client:        client,
		streamName:    streamName,
		consumerGroup: "state-run-entries-dispatch-failed",
	...
	logger.Info("RunEntriesDispatchFailedConsumer group ready", "group", c.consumerGroup, "stream", streamName)
	return c, nil
}

// Start begins consuming. Blocks until ctx is cancelled or Stop() is called.
func (c *RunEntriesDispatchFailedConsumer) Start(ctx context.Context) error {
	c.logger.Info("Starting RunEntriesDispatchFailedConsumer", "consumer", c.consumerName)
...
func (c *RunEntriesDispatchFailedConsumer) Stop() {
	close(c.stopCh)
}

func (c *RunEntriesDispatchFailedConsumer) processPending(ctx context.Context) error {
...
func (c *RunEntriesDispatchFailedConsumer) readAndProcess(ctx context.Context) error {
...
func (c *RunEntriesDispatchFailedConsumer) processMessage(ctx context.Context, msg goredis.XMessage) error {
