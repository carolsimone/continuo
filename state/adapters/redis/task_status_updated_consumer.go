...
// TaskStatusUpdatedConsumer consumes task.status.updated:v1 events and updates task state
// while performing run finalization when all tasks reach a terminal status.
type TaskStatusUpdatedConsumer struct {
...
// NewTaskStatusUpdatedConsumer creates and initialises the consumer group.
func NewTaskStatusUpdatedConsumer(
	client *goredis.Client,
	streamName string,
	db *sqlx.DB,
	...
	c := &TaskStatusUpdatedConsumer{
		...
		consumerGroup: "state-task-status-updated",
	...
	logger.Info("TaskStatusUpdatedConsumer group ready", "group", c.consumerGroup, "stream", streamName)
	return c, nil
}

// Start begins consuming. Blocks until ctx is cancelled or Stop() is called.
func (c *TaskStatusUpdatedConsumer) Start(ctx context.Context) error {
	c.logger.Info("Starting TaskStatusUpdatedConsumer", "consumer", c.consumerName)
...
func (c *TaskStatusUpdatedConsumer) Stop() {
	close(c.stopCh)
}

func (c *TaskStatusUpdatedConsumer) processPending(ctx context.Context) error {
...
func (c *TaskStatusUpdatedConsumer) readAndProcess(ctx context.Context) error {
...
func (c *TaskStatusUpdatedConsumer) processMessage(ctx context.Context, msg goredis.XMessage) error {
