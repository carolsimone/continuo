...
// RunRerunDispatchedConsumer consumes run.rerun.dispatched:v1 events and resets
// specified tasks to PENDING while reviving the run to RUNNING.
type RunRerunDispatchedConsumer struct {
	client        *goredis.Client
	streamName    string
	consumerGroup string
	consumerName  string
	handler       *RunRerunDispatchedHandler
	logger        *slog.Logger
	stopCh        chan struct{}
}

// NewRunRerunDispatchedConsumer creates and initialises the consumer group.
func NewRunRerunDispatchedConsumer(
	client *goredis.Client,
	streamName string,
	db *sqlx.DB,
	...
	logger *slog.Logger,
) (*RunRerunDispatchedConsumer, error) {
	handler := NewRunRerunDispatchedHandler(db, schedulerRepo, taskRepo, logger)
	c := &RunRerunDispatchedConsumer{
		client:        client,
		streamName:    streamName,
		consumerGroup: "state-run-rerun-dispatched",
	...
	logger.Info("RunRerunDispatchedConsumer group ready", "group", c.consumerGroup, "stream", streamName)
	return c, nil
}

// Start begins consuming. Blocks until ctx is cancelled or Stop() is called.
func (c *RunRerunDispatchedConsumer) Start(ctx context.Context) error {
	c.logger.Info("Starting RunRerunDispatchedConsumer", "consumer", c.consumerName)
...
func (c *RunRerunDispatchedConsumer) Stop() {
	close(c.stopCh)
}

func (c *RunRerunDispatchedConsumer) processPending(ctx context.Context) error {
...
func (c *RunRerunDispatchedConsumer) readAndProcess(ctx context.Context) error {
...
func (c *RunRerunDispatchedConsumer) processMessage(ctx context.Context, msg goredis.XMessage) error {
