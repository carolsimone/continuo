...
// ScheduleCatalogConsumer consumes schedules.loaded:v1 events and reconciles schedule_catalog.
type ScheduleCatalogConsumer struct {
...
// NewScheduleCatalogConsumer creates and initialises the consumer group.
func NewScheduleCatalogConsumer(
	client *goredis.Client,
	streamName string,
	...
	c := &ScheduleCatalogConsumer{
		...
		consumerGroup: "state-schedule-catalog",
	...
	logger.Info("ScheduleCatalogConsumer group ready", "group", c.consumerGroup, "stream", streamName)
	return c, nil
}

// Start begins consuming. Blocks until ctx is cancelled or Stop() is called.
func (c *ScheduleCatalogConsumer) Start(ctx context.Context) error {
	c.logger.Info("Starting ScheduleCatalogConsumer", "consumer", c.consumerName)
...
func (c *ScheduleCatalogConsumer) Stop() {
	close(c.stopCh)
}

func (c *ScheduleCatalogConsumer) processPending(ctx context.Context) error {
...
func (c *ScheduleCatalogConsumer) readAndProcess(ctx context.Context) error {
...
func (c *ScheduleCatalogConsumer) processMessage(ctx context.Context, msg goredis.XMessage) error {
	payloadStr, _ := msg.Values["payload"].(string)
	shouldACK, err := c.handler.Handle(ctx, payloadStr)
	if err != nil {
