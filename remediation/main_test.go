...
func TestHealthyRegistryPassesBothProbes(t *testing.T) {
	...
	reg.RegisterWorker("release_rejected")
	reg.AddWorkerProbe("release_rejected_heartbeat", 0, func(context.Context) error { return nil })
...
func TestConsumerExitFailsBothProbes(t *testing.T) {
	reg := liveness.NewRegistry()
	reg.RegisterWorker("release_rejected")
	reg.WorkerExited("release_rejected", errors.New("permanent bootstrap failure: WRONGTYPE"))
...
func TestStaleHeartbeatFailsBothProbes(t *testing.T) {
	reg := liveness.NewRegistry()
	reg.RegisterWorker("release_rejected")
	reg.AddWorkerProbe("release_rejected_heartbeat", 0, func(context.Context) error {
