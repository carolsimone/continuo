...
type FailureEvidence struct {
	...
	NodeType             string // optional; the failing node's kind (dbt-model, dbt-seed, dbt-snapshot, python-model, python-csv); set for validation, duplicate_table and parse failures — this is what selects the agent's fixer lane for a python node without a topology lookup of its own; empty for compile and seed_build
