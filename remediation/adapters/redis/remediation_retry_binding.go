...
// classifies its whole failed-node set exactly as the original rejection was,
// emitting one batched trigger for the round, so a "try again" walks the same
// path as the rejection itself, one round later.
