...
ALTER TABLE release_controller_outbox
...
-- covered by idx_release_controller_outbox_pending (created_at, WHERE
-- status='pending') from the canonical migration.
CREATE INDEX IF NOT EXISTS idx_release_controller_outbox_due
    ON release_controller_outbox (next_attempt_at)
...
ALTER TABLE release_controller_outbox ALTER COLUMN max_retries SET DEFAULT 13;
...
ALTER TABLE release_controller_outbox
    DROP CONSTRAINT IF EXISTS release_controller_outbox_status_check;
ALTER TABLE release_controller_outbox
    ADD CONSTRAINT release_controller_outbox_status_check
