...
ALTER TABLE remediation_agent_outbox
...
-- covered by idx_remediation_agent_outbox_pending (created_at, WHERE
-- status='pending') from the init migration.
CREATE INDEX IF NOT EXISTS idx_remediation_agent_outbox_due
    ON remediation_agent_outbox (next_attempt_at)
...
ALTER TABLE remediation_agent_outbox
    DROP CONSTRAINT IF EXISTS remediation_agent_outbox_status_check;
ALTER TABLE remediation_agent_outbox
    ADD CONSTRAINT remediation_agent_outbox_status_check
