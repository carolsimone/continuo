...
-- covered by idx_executor_outbox_pending (created_at, WHERE status='pending')
-- from the canonical migration.
CREATE INDEX IF NOT EXISTS idx_executor_outbox_due
    ON executor_outbox (next_attempt_at)
...
ALTER TABLE executor_outbox ALTER COLUMN max_retries SET DEFAULT 10;
...
ALTER TABLE executor_outbox
    DROP CONSTRAINT IF EXISTS executor_outbox_status_check;
ALTER TABLE executor_outbox
    ADD CONSTRAINT executor_outbox_status_check
