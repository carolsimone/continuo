...
//     Postgres) against a LIVE context derived from context.Background(),
//     never the root context that was just cancelled.
...
func (al *ApplicationLifecycle) Shutdown(cancel context.CancelFunc, grace time.Duration) {
	...
	// 1. Stop intake: cancel the root context so background jobs stop.
	cancel()
