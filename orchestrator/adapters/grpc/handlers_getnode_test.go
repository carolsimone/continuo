...
func TestGetNode_MapsMeta(t *testing.T) {
	h := NewQueryHandler(stubNodeReader{meta: &domain.NodeMeta{NodeType: "dbt-model", TestCount: 0, TestCountKnown: true}}, nil, testLogger())
...
func TestGetNode_NotFound(t *testing.T) {
	h := NewQueryHandler(stubNodeReader{err: domain.ErrNodeNotFound}, nil, testLogger())
...
func TestGetNode_MissingArgs(t *testing.T) {
	h := NewQueryHandler(stubNodeReader{}, nil, testLogger())
