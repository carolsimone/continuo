...
import { buildSwimlaneLayout } from './run-graph-helpers';
...
// small `.swim-cell` squares instead of full node boxes.
export default function RunSwimlane({
  ...
  const layout = buildSwimlaneLayout(nodeIds, graph.edges, serviceOrder, collapsed);
  ...
  return (
    <div className="swim-wrap">
      <div className="swim-stage" style={{ position: 'relative', width: layout.width, height: layout.height }}>
        ...
        {layout.bands.map((b) => {
          const { done, total, roll } = laneRollup(b.service);
          return (
            <div
              key={b.service}
              className="swim-lab"
        ...
        {layout.nodes.map((n) => {
          ...
          return (
            <div
              ...
              title={n.nodeId}
              style={{ position: 'absolute', left: n.x, top: n.y, width: NODE_W, borderColor: colors.get(n.service) }}
            >
