...
func TestReadyAndLivezHealthyWhenRegistryHealthy(t *testing.T) {
	...
	reg.AddWorkerProbe("check_k8s_heartbeat", 0, func(context.Context) error { return nil })
...
func TestConsumerExitFailsBothProbes(t *testing.T) {
	reg := liveness.NewRegistry()
	reg.RegisterWorker("check_k8s")
	reg.WorkerExited("check_k8s", errors.New("permanent bootstrap failure: WRONGTYPE"))
	srv := NewHealthServer(0, reg, discardLogger())
...
func TestStaleHeartbeatFailsBothProbes(t *testing.T) {
	reg := liveness.NewRegistry()
	reg.RegisterWorker("check_k8s")
	reg.AddWorkerProbe("check_k8s_heartbeat", 0, func(context.Context) error {
