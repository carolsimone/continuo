...
// ShadowReleaseID mints the id of the release that verifies one attempt's
// edits to one service. It embeds the failing release, the edited service, and
// the attempt, so it is unique per (service, attempt) and legible in release
// listings, and so a redelivery of the same attempt reuses the same id —
// release-controller's submission is idempotent on it.
//
// Past maxShadowReleaseIDLen the middle — the failing release and service — is
...
func ShadowReleaseID(releaseID, service string, attempt int) string {
