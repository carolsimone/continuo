...
// (release, node, attempt) and distinct across attempts and from PROpened ids.
func TestPRClosedEventID_Deterministic(t *testing.T) {
	a := PRClosedEventID("rel-1", "model.p.orders", 1)
	b := PRClosedEventID("rel-1", "model.p.orders", 1)
	require.Equal(t, a, b, "same inputs must produce the same event id")
	require.NotEqual(t, a, PRClosedEventID("rel-1", "model.p.orders", 2))
	require.NotEqual(t, a, PROpenedEventID("rel-1", "model.p.orders", 1),
...
func TestPRClosed_JSONShape(t *testing.T) {
	...
	for _, k := range []string{"proposal_id", "release_id", "node_id", "pr_url", "pr_number", "outcome", "closed_at"} {
