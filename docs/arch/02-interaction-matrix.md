...
|---|---|---|
| `manifest-controller` | read + write | `download_file` (the `manifest.json` files named in the `release.requested:v1` `manifest_keys` list; no S3 listing); `PutObject` (candidate SQL per non-seed node to `candidate-sql/<release_id>/candidate_<unique_id>.sql`; upload failure aborts the load); `PutObject` (one code-bundle contract document per release to `code-bundles/<release_id>/bundle.json`; upload failure aborts the load) |
...
| `release-controller` | delete | `DeleteObjects` — prune-time delete of `candidate-sql/<release_id>/` (soft-fail; S3 lifecycle rule is the backstop) and `code-bundles/<release_id>/` (soft-fail; no lifecycle backstop — prune is the only reclaim path) prefixes for each expired release |
