...
| `ui-service` | read | `GetObject` — task-execution pod logs (proxied via `GET /api/task-executions/:id/logs`); dbt validation logs (proxied via `GET /api/releases/log`); every proposal edit's corrected content (one `GetObject` per `content_uri` on the claim's `edits` list, fetched during Create PR to supply each file's content to GitHub) |
