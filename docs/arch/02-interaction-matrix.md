...
| `remediation-agent` | `RW` (`continuo_remediation_agent`) | `-` | `RW` | `-` | `R` (`GetNodeLocation`, `GetUpstreamChanges`, `GetNodeVersions`, `GetPrecedents` — one `GraphClient`, each best-effort; see narrative below for which fixer calls which) | `-` | `-` | `RW` (read candidate SQL, dbt log, and — validation only — the release code bundle; write `proposed-fix/` artifacts) | `-` | `W` (Anthropic or OpenAI-compatible HTTPS) | `R` (Contents API, file + directory reads at `commit_sha` — offending-file reads for compile/seed/duplicate_table, and validation's fallback on a permanent code-bundle miss; Pulls API, PR-status polling by the reconciler; all read-only) | `-` | server (port 50054) |
| `continuo CLI` | `-` | `-` | `-` | `R` | `R` | `-` | `-` | `-` | `-` | `-` | `-` | `-` | `-` |
...
| `release.rejected:v1` | `release-controller` | `remediation` (group `remediation-release-rejected`) | A release failed parsing, validation, or pre-validation checks (e.g. `unbuildable_cross_service_upstream`, `duplicate_table`); consumed by the remediation classifier to triage each failing node. |
