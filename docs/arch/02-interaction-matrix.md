...
| `trigger.single_node_run:v1` | `state` (outbox processor on `TriggerSingleNodeRun` gRPC call) | `orchestrator` | Request a one-task run; orchestrator's `HandleSingleNodeRun` runs `Snapshot(SingleNode)` and emits `run.entries.dispatched:v1` + one `query.model:v1`. Latest mode reads the node's metadata — image tag, manifest version, `dbt_unique_id` and the runtime manifest reference — from `:TopologyRoot`; `snapshot_of_run` mode reads the same set from the source `:Run`'s `:EXECUTES` edge. Either way the snapshot pins it onto the new run's `:EXECUTES` edge and the `query.model:v1` dispatch carries it |
| `query.model:v1` | `orchestrator` | `executor-controller` | Dispatch executable nodes; carries image_tag, manifest_version, and operation (`""`/`test`/`build`) as stream fields, on every dispatch path (initial frontier, downstream unblock, derived-run frontier). A node pinned to a runtime manifest additionally carries `dbt_unique_id` and the four `runtime_manifest_*` fields, read from the run's `:EXECUTES` edge on each of those paths and on the single-node run; a node with no pin omits all five, and executes down the per-node Job path. The seed-build dispatch on `release.promoted:v1` snapshots no run and carries no pin |
