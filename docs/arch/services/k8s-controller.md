...
`CheckStatusHandler` fetches the live Job's labels and routes on `mode`. A Job labelled `mode=validation` (created by `executor-controller`'s validation dispatch path, also carrying `release-id` and `node-id` labels) bypasses the production outcome branches entirely:

- **Running / Unknown** — not terminal; the handler returns without writing any outbox row. The Job is re-checked later through the normal `check.k8s:v1` re-enqueue path; no result is emitted while the Job is in flight.
- **Succeeded / Failed** — the handler uploads pod logs (soft-fail) and writes exactly ONE `validation_node_completed` outbox row (→ `validation.node.completed:v1`) carrying `{release_id, node_id, outcome, dbt_log_uri}`. `outcome` is `ok` when the Job succeeded, else `failed`; `release_id`/`node_id` come from the Job labels. This single row replaces the three production task-status rows — no `task_status_updated`, `task_execution_recorded`, or `node_status_updated` is written for validation Jobs.
