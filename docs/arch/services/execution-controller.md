...
| `deployments` | The deploy command queue and the per-node candidate-leg state store. A `mode` column distinguishes `production` rows (the default `query.model:v1`/`retry.task:v1` path) from `validation`/`seed_build`/`compile` rows (candidate-release legs, which carry `release_id`/`node_id` and a per-node terminal `outcome`) |
...
| `retry.task:v1` | `executor-retry` | Outbox-entry-carried | Retry dispatch: re-attempt a failed node, produced and consumed by this service |
...
| `validation.node.completed:v1` | `executor-validation-node-completed` | Standard `(message_id, stream_name)` with an `outbox_entry_id` fallback | Per-node validation Job terminal status from the job-status handler; records the node outcome, unblocks or skips in-set downstreams, and runs the per-release aggregate-emit gate |
...
| `seed.build.node.completed:v1` | `executor-seed-build-node-completed` | Standard with `outbox_entry_id` fallback | Per-seed build terminal status from the job-status handler; records outcome and runs the per-release seed-build aggregate-emit gate |
...
| `compile.node.completed:v1` | `executor-compile-node-completed` | Standard with `outbox_entry_id` fallback | Compile Job terminal status from the job-status handler; records outcome and emits `compile.completed:v1` via the aggregate gate |
...
| `retry.task:v1` | Published by the job-status handler when a production Job fails with retry budget remaining; a new `deployments` row for the retry is created via `retry_task_handler.go` at the next attempt's `-rN` job name |
| `task.failed:v1` | Published when a Job's status is Unknown (treated as a permanent failure); declared with no consumer in the repository — kept for external observability |
| `validation.node.completed:v1` | Published by the job-status handler on a `mode=validation` Job terminal; a single per-node result row instead of the three production task-status rows |
| `seed.build.node.completed:v1` | Published by the job-status handler on a `mode=seed_build` Job terminal |
| `compile.node.completed:v1` | Published by the job-status handler on a `mode=compile` Job terminal; carries `failed_container` when the terminal result attributes the failure to a specific pod container |
...
`operation` (the dbt verb the Job runs, e.g. `test`, `build`) is sourced from durable check/retry data, not from Job metadata: it arrives on `node.deployed:v1`, is held on the check command, rides the delay-queue ticket onto every `check.k8s:v1` self-poll, and is copied onto `retry.task:v1`. Sourcing it from the Job's labels would be unsafe because a TTL-reaped ("vanished") Job returns empty labels, which would silently rebuild a `dbt test`/`dbt build` Job as `dbt run`; carrying it in the durable payload keeps a retried `dbt test` or `dbt build` Job the same verb. Normal production runs have an empty `operation`, so the field is omitted and the wire format is unchanged for them.
...
A candidate row is dispatched only once `pending` (unblocked); `blocked` rows (with unresolved in-set upstreams) remain in place until the node-completed handler transitions them to `pending`. The dispatcher never dispatches `blocked` rows.
...
| **Failed, retryable** (`retry_count < max_retries`, production) | Fetches and uploads logs → writes `task_status_updated` (FAILED) stamping the attempt that ran, `task_execution_recorded`, and `retry.task:v1` for the next attempt (`retry_count + 1`) with the `-rN` job name |
...
| **Unknown** (production) | Not retryable state: writes `task_status_updated` (FAILED) and `task.failed:v1` with the pod's termination message or a default reason |
| **`mode=validation` terminal** | An Unknown status here is treated as not-yet-terminal and re-polled via `check.k8s:v1` rather than failed. On Succeeded/Failed: fetches and uploads logs, derives `outcome` (`ok`/`failed`), reads `release_id`/`node_id` from the Job's raw `continuo.dev/*` annotations, and writes a single `validation_node_completed` outbox row keyed on a UUIDv5 over an immutable namespace and `release:<release_id>` |
| **`mode=seed_build` terminal** | Same shape as validation, writing a single `seed_build_node_completed` row under its own namespace |
| **`mode=compile` terminal** | Same shape, writing a single `compile_node_completed` row (also carrying `failed_container` when the terminal result attributes the failure to a specific pod container) under its own namespace; the compile leg's log and run-results are filed under `compileArtifactPath` (`{service}/compile`), not a node path |
...
| Loop | Description |
|---|---|
| Redis consumers (15 streams) | One tracked goroutine per stream, registered with the liveness registry as `query_model`, `retry_task`, `schedule_cancelled`, `validation_requested`, `validation_node_completed`, `seed_build_requested`, `seed_build_node_completed`, `compile_requested`, `compile_node_completed`, `validation_result_teardown`, `pipeline_run_finished_teardown`, `release_promoted_teardown`, `release_rejected_teardown`, `node_deployed`, `check_k8s`; each carries a heartbeat probe and, for the six schema-op consumers, a longer handler timeout and pending-entry reclaim window (see Kubernetes API → `RunSchemaOpJob`) |
...
- **Cancelled-schedule guard**: a `cancelled_schedules` row, checked both at dispatch (`QueryModelHandler`/`RetryTaskHandler`) and at Job-status time (the job-status handler), drops a schedule's dispatch or in-flight Job result without writing a `deployments` row or an outbox announcement; rows are swept after a configurable TTL.
