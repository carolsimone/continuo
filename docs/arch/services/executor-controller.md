...
| `parse` | `dbt parse --profiles-dir /project` — the parse-export/rehearsal leg run by the compile Job's `parse-prod`/`parse-candidate` initContainers (see `CreateCompileJob` below); takes no placeholders |
