...
- `CreateValidationJob` — creates a `mode=validation` K8s batch Job in the configured namespace (idempotent by job name, `BackoffLimit` 0, `RestartPolicy` Never). Labels: `app=dbt-job` (so existing watchers stay correct) plus `mode=validation` and `release-id`/`node-id` (sanitised to valid label values) for selection/observability; the `mode` label is what k8s-controller routes on. The raw, unmodified release/node identity is stamped separately as annotations `continuo.dev/release-id` and `continuo.dev/node-id` (allowing arbitrary values); k8s-controller reads these — not the sanitised labels — into the `validation.node.completed:v1` payload so the outcome lookup keyed on the unmodified `executor_deployments` row matches even when a label would be altered. The container command comes from `ValidationDbtCommand` — `dbt run/seed/snapshot --select <table> --empty`. Topological gating ensures every in-set upstream (intra- and cross-service) has completed in the shared `_candidate_<release>` schema before any dependent node runs; no node reads a production table for its upstream data. Schema redirection uses two env vars set on every validation pod: `DBT_TARGET_SCHEMA` overrides the output (materialization) schema via the `generate_schema_name` macro baked into `dbt-base`; `DBT_UPSTREAM_SCHEMA` overrides the input (source read) schema via an inline `env_var('DBT_UPSTREAM_SCHEMA', target.schema)` in cross-service `FROM` references (`FROM {{ env_var('DBT_UPSTREAM_SCHEMA', target.schema) }}.<table>`) — when unset in CI or production runs it resolves to the normal schema, and on validation pods it resolves to the candidate schema, redirecting cross-service `FROM` references there. Both env vars are set to the same candidate schema (`_candidate_<release>`). Env mirrors the query Job's `DBT_POSTGRES_*` connection forwarding and adds `DBT_TARGET_SCHEMA`, `DBT_UPSTREAM_SCHEMA`, `RELEASE_ID`, `NODE_ID`. A missing `image_tag` is a permanent error.
