...
| `validation.result:v1` (`kind=complete`) | Per-release validation decision; emitted exactly once when every `mode=validation` node for a release is terminal, and last on the stream (after every `kind=node` message for the release, under the same `aggregate_id`). Payload is the decision only: `kind` (`"complete"`), `release_id`, `candidate_schema` (the `_candidate_<release>` schema name, for teardown), and `aggregate_status` (`ok` iff every node is `ok`, else `failed`). It carries no per-node content — those arrive as the earlier `kind=node` messages, which release-controller has already projected into its read model by the time this terminal message arrives. |
| `validation.result:v1` (`kind=node`) | Per-node validation projection; emitted in `SettleNodeTerminal` (validation leg) as each node settles, so `release-controller` can render per-node validation results live and build its per-node read model. One emit is produced for the settled node itself, plus one for every downstream node the settle's failure propagation skips (a failed node's transitive blocked descendants never run through their own settle, so the settle emits their projection on their behalf, with `status` `"skipped"` and no URIs). Each is an idempotent last-write projection; all `kind=node` messages and the trailing `kind=complete` share the release's `aggregate_id`, so PerAggregateFIFO delivers every node before the decision and release-controller needs no completeness barrier. Payload: `kind` (`"node"`), `release_id`, `stage` (`"validation"`), `node_id`, `status` (`ok`/`failed`/`skipped`), optional `dbt_log_uri`, optional `run_results_uri` |
