...
| `worker.wrapper_cache` | `required` when the team's wrapper reliably writes a reusable partial parse at that path, `opaque` when its cache behaviour is unknown. Absent, or an absent `worker` block, resolves to `opaque` — continuo only assumes a reusable parse cache when a team declares one. `worker` follows the same service-override-beats-`default` precedence as the command keys. |
