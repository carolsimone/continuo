...
Before enqueuing any node, the binding creates the release's `_candidate_<release>` schema in the dbt warehouse exactly once via `CandidateSchemaCreator.EnsureCandidateSchema`. Creation runs inside a warehouse transaction that first takes a transaction-scoped advisory lock (`pg_advisory_xact_lock`) keyed on the schema name, so concurrent callers for the same schema serialize and only one issues the `CREATE SCHEMA IF NOT EXISTS`; a unique-violation is additionally tolerated as success. Pre-creating the schema once, ahead of the fan-out, means every validation Job — including seeds, which run `dbt seed --empty` and create the schema through dbt's own non-atomic path — finds the schema already present, so two parallel root Jobs never race `CREATE SCHEMA` on `pg_namespace`. Schema creation runs against the warehouse outside the executor's Unit-of-Work transaction; a failure returns before any deployment row is enqueued and the message is retried. The pre-create runs only for a fresh release: a deduped redelivery skips it.
...
- `CreateQueryJob` — creates a K8s batch Job in the configured namespace with label `app=dbt-job`; container name is `dbt-job`; treated as idempotent (already-exists is not an error on retry). The container `imagePullPolicy` is `IfNotPresent`: production deploy/query images are referenced by content-addressed tags that are unique per build, so a cached image of a given tag is always the correct one.
- `CreateSeedBuildJob` — creates a `mode=seed_build` K8s batch Job (idempotent by job name). Uses the team service image (`imagePullPolicy: IfNotPresent`) and runs `dbt seed --select <TableName>` with `DBT_TARGET_SCHEMA=<CandidateSchema>` so the `generate_schema_name` macro in dbt-base routes the seed output to the candidate schema.
- `CreateCompileJob` — creates a `mode=compile` K8s batch Job (idempotent by job name). The Job is a two-container pod with a shared `emptyDir` volume (`shared`, mounted at `/shared`). The initContainer (`compile`) uses the team service image (`<service>:<image_tag>`, `imagePullPolicy: IfNotPresent`) and runs `dbt compile --profiles-dir /project && cp /project/target/manifest.json /shared/manifest.json` with `DBT_POSTGRES_*` env vars. The main container (`upload`) uses the shared `s3-sidecar` image (a minimal python+boto3 image with no dbt; hosts both `compile_uploader.py` for the compile leg and `candidate_fetcher.py` for the validation leg) configured via the `S3_SIDECAR_IMAGE` env var (defaults to `<DOCKERHUB_USERNAME>/s3-sidecar:latest`) and runs `python /compile_uploader.py` with `COMPILE_MANIFEST_PATH=/shared/manifest.json`, `MANIFEST_S3_URI`, and S3 credential env vars; its pull policy is governed by `VALIDATION_IMAGE_PULL_POLICY`. A missing `image_tag` is a permanent error.
- `CreateValidationJob` — creates a `mode=validation` K8s batch Job in the configured namespace (idempotent by job name, `BackoffLimit` 0, `RestartPolicy` Never). The `imagePullPolicy` defaults to `Always`: the continuo-owned `dbt-base` validation image is published under a mutable tag and re-pushed whenever the validation runner changes, so the node must re-pull to avoid using a stale cached layer. The policy is overridable via `VALIDATION_IMAGE_PULL_POLICY` (`IfNotPresent`/`Never`/`Always`) for clusters that side-load images and have no registry to pull from — the kind-based e2e suite and local dev set `IfNotPresent`, where `Always` would fail with `ErrImagePull`. Labels: `app=dbt-job` (so existing watchers stay correct) plus `mode=validation` and `release-id`/`node-id` (sanitised to valid label values) for selection/observability; the `mode` label is what k8s-controller routes on. The raw, unmodified release/node identity is stamped separately as annotations `continuo.dev/release-id` and `continuo.dev/node-id` (allowing arbitrary values); k8s-controller reads these — not the sanitised labels — into the `validation.node.completed:v1` payload so the outcome lookup keyed on the unmodified `executor_deployments` row matches even when a label would be altered. The pod spec is determined by `ValidationOp`: **`build_from_sql`** (default) — a two-container pod with a shared `emptyDir` volume (`shared`, mounted at `/shared` in both containers). The initContainer (`fetch`) uses the shared `s3-sidecar` image (configured via `S3_SIDECAR_IMAGE`, defaults to `<DOCKERHUB_USERNAME>/s3-sidecar:latest`; the same image used by the compile leg) and runs `python /candidate_fetcher.py`, which downloads the node's compiled SQL from S3 at `CANDIDATE_SQL_URI` and writes it to `CANDIDATE_SQL_PATH=/shared/candidate.sql`; it receives `S3_ENDPOINT_URL`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, and `AWS_DEFAULT_REGION`. The main container (`dbt-job`) uses `dbt-base` with **no S3 credentials and no `CANDIDATE_SQL_URI`**; it mounts `/shared` and reads the SQL from the local file at `CANDIDATE_SQL_PATH`. **`clone_from_prod`** (any other `ValidationOp`) — a single `dbt-job` container using `dbt-base` with no emptyDir and no S3 credentials; the runner clones the schema directly from production. The container command is `ValidationCommand(nodeType, tableName)`: for **model and snapshot** nodes it is `python /validation_runner.py`, which reads the candidate SQL from `CANDIDATE_SQL_PATH` and issues `CREATE SCHEMA IF NOT EXISTS <candidate>` + `CREATE TABLE <candidate>.<table> AS (<SQL>) WITH NO DATA`; for **seed** nodes it is `python /seed_validation_runner.py --select <table>`, a thin wrapper that runs `dbt seed --select <table> --empty` (building an empty seed table from the CSV column definitions) and then projects `target/run_results.json` into the structured result block. Both entrypoints print the same sentinel-framed structured validation-result block (`status`/`message`/`failures`/`unique_id`) as their last stdout, which k8s-controller uploads to S3 as `run_results_uri`. The candidate schema is created once, race-safely, by executor-controller before the fan-out (see the `validation.requested:v1` handling above), so by the time any Job runs the schema already exists; `validation_runner.py` still creates it under a session-level advisory lock (`pg_advisory_lock`) and tolerates `DuplicateSchema`/`UniqueViolation` as a second line of defense. Topological gating ensures every in-set upstream (intra- and cross-service) has been built as an empty table in the shared `_candidate_<release>` schema before any dependent node runs; because the SQL written to `CANDIDATE_SQL_PATH` already has every known-node schema ref rewritten to the candidate schema, no model edits and no dbt recompile are needed. Env vars set on every validation main container: `DBT_TARGET_SCHEMA` (= candidate schema, consumed by the `generate_schema_name` macro in `dbt-base` for the seed path's output schema), `CANDIDATE_SQL_PATH` (path to the local SQL file; default `/shared/candidate.sql`), `TABLE_NAME`, `RELEASE_ID`, `NODE_ID`, `SERVICE_NAME`, `SCHEMA`, `JOB_NAME`, `VALIDATION_OP`, `PROD_SCHEMA`, plus `DBT_POSTGRES_*` connection forwarding. S3 credentials (`S3_ENDPOINT_URL`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_DEFAULT_REGION`) and `CANDIDATE_SQL_URI` are set **only on the `fetch` init container** of `build_from_sql` pods; the `dbt-base` main container carries no S3 credentials. A missing `image_tag` is a permanent error.
