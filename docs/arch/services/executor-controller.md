...
| `validation.node.result:v1` | Per-node validation projection; emitted once per `mode=validation` node in `SettleNodeTerminal`, as that node settles, so `release-controller` can render per-node validation results live. Each emit is an idempotent last-write projection with a distinct `aggregate_id` (not a deduped decision like `validation.completed:v1`); the promote/reject decision still derives solely from the `validation.completed:v1` aggregate. Payload: `release_id`, `stage` (`"validation"`), `node_id`, `status` (`ok`/`failed`), optional `dbt_log_uri`, optional `run_results_uri` |
