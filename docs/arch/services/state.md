...
servers, cron scheduler, Postgres, Redis) against a fresh live context,
sharing a single deadline with step 2 so infra teardown always retains at
least the other half of the budget, derived from `context.Background()`,
never the just-cancelled root context. `main` blocks
...
| `release.seeds.pending:v1` | `state-release-seeds-pending` | `PromotedSeedsHandler` — creates the run that materialises the seeds a release changed. orchestrator writes this event inside its topology-swap transaction and only when at least one seed changed, so the nodes already exist and there is no empty-run case here. The run id is derived deterministically from the release id, so a redelivered event resolves to the same run instead of rebuilding seeds already built. Emits `trigger.promoted_seeds:v1` for orchestrator to project the tasks. |
