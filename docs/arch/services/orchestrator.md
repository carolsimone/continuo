...
Building the seeds a release changed is a separate flow: `state` consumes `release.promoted:v1` on its own group, creates a run for those seeds, and emits `trigger.promoted_seeds:v1`, which orchestrator consumes to project that run's tasks. The seeds are built by an ordinary run, so a failed seed build is retried, recorded, and visible like any other task.
