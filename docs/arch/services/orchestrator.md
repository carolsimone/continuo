...
| `rejection_unique` | constraint, `:Rejection(release_id, node_id)` unique | A rejection's identity, matching the classifier's dedup key, so a redelivered `remediation.requested:v1` MERGEs onto the row it already wrote. |
...
### On `remediation.requested:v1` — RemediationRequestedRejectionsHandler

Consumes `remediation.requested:v1` on the case-base group (`orchestrator-remediation-requested-rejections`, independent of `agent-remediation`'s own group on the same stream) and records each classified failure as a `:Rejection`. No outbox entry is produced.

1. Dedup on `message_processing`, scoped by the consumer-group name.
2. Parses `release_id`, `node_id`, `error_signature` (all required — a permanent error otherwise) and `classified_at` (RFC3339, permanent error if unparseable).
3. Resolves the failing candidate's code: empty `code_bundle_uri` (a compile-stage failure, which precedes the parse that produces a bundle) records the rejection with no code and no retry; a bundle absent from object storage but possibly still landing is a plain error, left in the PEL for redelivery; a bundle that fetches but is malformed, has no entry for this `node_id`, or belongs to a different release than `release_id` (the URI resolved to another release's bundle — unlike `ReleasePromotedVersionsHandler`, which drops the message in this case because writing the wrong code would corrupt the version graph, this handler is code-optional by design) is logged and the rejection is still recorded without code — losing the code must never cost the precedent.
4. `CaseBaseRepository.RecordRejection` upserts the `:Rejection` (completing a proposal-created stub if one exists), MERGEs its `:ErrorSignature` hub and `[:HAS_SIGNATURE]` edge, anchors `[:FAILED {release_id}]` from the node's `:Table` when one exists, and back-links `[:RESOLVED_BY]` when no `[:RESOLVED_BY]` exists yet — to the oldest version already promoted after the rejection, or, for a revert, to the node's `:CURRENT` version — see "Failure-precedent case base — graph model" above.
...
| Redis consumer (`remediation.requested:v1` rejections) | Reads `remediation.requested:v1` on consumer group `orchestrator-remediation-requested-rejections`; dispatches to `RemediationRequestedRejectionsHandler` |
