...
| Entity | Description |
|---|---|
| `Table` node | One per model/seed; carries topology metadata (`schema_name`, `table_name`, `service_name`, `node_type`, `test_count`, `schedule_name`, `image_tag`, `original_file_path`), the node's current `content_hash`, `last_updated_at`, and an `active` flag for current-topology reconciliation. `content_hash` is refreshed on every promotion, changed or not, and is what the code-version path compares a release's code bundle against. Code-change provenance (commit, repo, promotion time) is not stamped on `:Table` — it lives on `:NodeVersion`, reached from `:Table` via `CURRENT` (see "Code-version history — graph model" below). `test_count` is the number of dbt tests declared for the node, carried on `release.promoted:v1` and defaulted to `0` when absent (`COALESCE(t.test_count, 0)` on read). |
...
| `NodeVersion` node | One immutable code state of a node, identified by (`unique_id`, `content_hash`); pointed at from its `:Table` by `CURRENT`. See "Code-version history — graph model" below. |
...
| `Rejection` node | One classified failure of one node in one rejected release, identified by (`release_id`, `node_id`). Carries `stage` (`parse`, `validation`, `seed_build`, `compile`, or `duplicate_table`), `category`, `reason`, `error_excerpt`, `dbt_log_uri`, `at` (the classifier's `classified_at`), `raw_code`, `content_hash` (both empty for a compile-stage or parse-stage failure — neither ever reaches a completed parse, so neither has a code bundle to read from), and `stub` (`true` for a placeholder a proposal created ahead of its rejection, completed to `false` once the rejection itself lands). See "Failure-precedent case base — graph model" below. |
...
| `PullRequest` node | One fix PR's facts, identified by (`proposal_id`, `service`) — a proposal that touches several services gets one `:PullRequest` per service. Carries `pr_url`, `pr_number`, `pr_state` (`open` at creation, advanced to `merged` or `rejected` when the PR reaches its terminal outcome), `opened_by`, `opened_at`, and `closed_at` (set when the terminal outcome lands). The open (`remediation.pr_opened:v1`) and close (`remediation.pr_closed:v1`) events are consumed by independent groups, so either can MERGE this node first: the open write fills every open fact, the close write ON-CREATE-fills the `pr_url`/`pr_number` its own event carries plus `pr_state`/`closed_at`, and whichever lands second fills the rest ON MATCH — the open write backfills `opened_by`/`opened_at` (and `pr_url`/`pr_number` only while still blank) but never overwrites a terminal `pr_state` back to `open`. Either update MERGEs the same node on (`proposal_id`, `service`) — never a second `:PullRequest`. Pre-existing `:Proposal` nodes written before this split still carry their own `pr_url`/`pr_number`/`pr_state`/`opened_by`/`opened_at` inline — those legacy properties are read, never written, going forward. |
| `HAS_SIGNATURE` relationship | `(:Rejection)-[:HAS_SIGNATURE]->(:ErrorSignature)` — links a rejection to its global signature hub. |
| `PROPOSED` relationship | `(:Rejection)-[:PROPOSED {service}]->(:Proposal)` — links a rejection to the fix proposed for it; `service` is the service whose PR proposes this node's fix. Because a batched proposal spanning several services is shared by all their rejections and carries one `:PullRequest` per service, `service` scopes a precedent read of one rejection to only its own service's `:PullRequest` facts — without it, every service's PR would fan into every rejection's `proposals` list. A legacy edge written before the stamp existed has a null `service` and falls back to matching any of the proposal's PRs. |
| `HAS_PR` relationship | `(:Proposal)-[:HAS_PR]->(:PullRequest)` — links a proposal to the PR facts for the service it fixes. |
| `RESOLVED_BY` relationship | Two target types, written by different consumers. `(:Rejection)-[:RESOLVED_BY]->(:NodeVersion)` — links a rejection to the code version that resolved it. Two consumers converge on this edge from opposite directions, each guarded on "no existing `RESOLVED_BY`" so whichever reaches an open rejection first wins. The versions consumer forward-links every still-open rejection of a node the moment a new version becomes that node's `CURRENT`, and stamps the edge (`ON CREATE`) with `promoted_at`/`release_id` from that promotion — the detail read path uses the edge's own `promoted_at` as its diff baseline, since a reverted-to `:NodeVersion` keeps its original, earlier `promoted_at` forever. The rejections consumer back-links when the fix already landed before the rejection was recorded, in two cases: ordinarily, the oldest `:NodeVersion` whose own `promoted_at` is after the rejection; for a revert (a promotion reusing an existing, older version node), detected instead through the node's `:CURRENT` edge, whose `promoted_at` the version writer always overwrites. A revert followed by a further promotion can still lose attribution to the version that actually reintroduced the fix — recovering full promotion history is a known gap, not attempted here. `(:Rejection)-[:RESOLVED_BY {amended, service}]->(:Proposal)` — the case-base provenance edge, written by the provenance consumer for every resolved node when its fix PR merges; `amended` is whether a human changed any of the PR's edits before merge, and `service` is the service whose PR resolved this node. Because a batched proposal spanning several services is shared by all their rejections, `service` is what scopes a precedent read of one rejection to only its resolving service's `[:EDITED]` edges and `:PullRequest` (see the read paths below); a legacy edge written before the stamp existed has a null `service` and falls back to the whole-proposal walk. |
| `EDITED` relationship | `(:Proposal)-[:EDITED {path, amended, diff, service}]->(:Table)` — one per file a merged fix PR edited, from the shared `:Proposal` to the edited node's `:Table`. Keyed on `path`; carries whether a human `amended` that edit before merge, the amendment `diff`, and the owning `service`. Written only when the target `:Table` exists; this writer never mints one. |
...
| `code_unit_version_unit_id` | index, `:CodeUnitVersion(unit_id)` | The `unit_id`-only lookups `GetCodeUnitVersions` issues (`UnitVersions`, and the node-selector path's batched round trip — see below). `code_unit_version_unique` only covers the composite `(unit_id, checksum)`, so without this index Neo4j plans a full `:CodeUnitVersion` label scan for every one of those reads. |
| `rejection_unique` | constraint, `:Rejection(release_id, node_id)` unique | A rejection's identity, matching the classifier's dedup key, so a redelivered `remediation.requested:v2` MERGEs onto the row it already wrote. |
| `rejection_node_id` | index, `:Rejection(node_id)` | `[:RESOLVED_BY]` linking from both directions: the versions consumer's forward-link and the rejections consumer's own back-link. |
| `rejection_category_reason` | index, `:Rejection(category, reason)` | `GetPrecedents`' fallback lookup when no signature is given — filters on the `:Rejection`'s own (always-current) properties, not the `:ErrorSignature` hub's first-seen ones. |
...
| `error_signature_category_reason` | index, `:ErrorSignature(category, reason)` | Backs no read path — the category+reason precedent lookup filters `:Rejection` (see `rejection_category_reason`) — but is retained: `:ErrorSignature` nodes are few (one per distinct signature), so the write cost is negligible, and the hub's category/reason are accurate first-seen metadata worth indexing for direct hub lookups. |
...
- The first normalizes any uppercase `:Run.terminal_status` (`SUCCEEDED`/`FAILED`) to the canonical lowercase form so the UI never reads a mixed casing: `MATCH (r:Run) WHERE r.terminal_status IN ['SUCCEEDED','FAILED'] SET r.terminal_status = toLower(r.terminal_status)`. Re-running it is a no-op once every row is lowercase.
- The second deletes every `[:PREVIOUS]` relationship in the graph: `MATCH ()-[p:PREVIOUS]->() DELETE p`. Re-running it finds no edges and is a no-op.
- The third removes the `last_commit_sha`/`last_repo`/`last_changed_at`/`last_release_id` properties from every `:Table`: `MATCH (t:Table) WHERE t.last_commit_sha IS NOT NULL OR t.last_repo IS NOT NULL OR t.last_changed_at IS NOT NULL OR t.last_release_id IS NOT NULL REMOVE t.last_commit_sha, t.last_repo, t.last_changed_at, t.last_release_id`. Version provenance lives on `:NodeVersion`, and no reader consults these `:Table` properties; leaving them in place would serve a stale commit to a reader. `REMOVE` on an absent property is a no-op, so re-running it is idempotent.
...
| `release.promoted:v1` | `orchestrator-release-promoted` | `ReleasePromotedHandler` — swaps the `:Table` topology via `ReleasePromotionRepository.PromoteRelease`, refreshes every node's `content_hash` (changed or not), increments `topology_generation`, writes `:TopologyRoot` service_metadata, then emits `schedules.loaded:v1` |
...
| `remediation.pr_closed:v1` | `orchestrator-remediation-pr-closed-provenance` | `PrClosedProvenanceHandler` — stamps a fix PR's terminal `pr_state`/`closed_at` on its `:PullRequest`, and on a **merged** outcome draws the case-base provenance edges: `[:RESOLVED_BY {amended, service}]` from every resolved node's `:Rejection` to the shared `:Proposal`, and `[:EDITED {path, amended, diff, service}]` from that `:Proposal` to each edit's `:Table` (skipped when the `:Table` is absent). A **rejected** outcome updates only the terminal state. No outbox emission |
...
| `ListScheduleTopologies` | Returns one entry per schedule with at least one active `:Table`: `schedule_name`, `node_count`, `last_updated_at = max(:Table.last_updated_at)`. Backs the ui homepage `Topology` tab tile grid. |
...
| `GetNodeLocation` | Returns where a node's source lives — `file_path` (dbt `original_file_path`, project-relative; `""` if not captured) and `service_name` — for a single active `:Table` addressed by `unique_id`. Returns `NOT_FOUND` for an unknown or inactive `unique_id`. Not cached — a single-row property lookup. |
| `GetNodeVersions` | Returns a node's recorded code-version history, newest first (ordered by `promoted_at` — when each version's code was *first promoted*; a revert that re-points the running code at an existing version does not move it in this ordering, only `is_current` moves). `limit` <= 0 defaults to 20, clamped to a max of 200. `include_code` (default `false`) controls whether `raw_code`/`compiled_code` are fetched from Neo4j at all — a single version's `compiled_code` can run to 256 KiB, so a caller that only needs hashes/provenance leaves it unset. `current_only`, when `true`, returns only the version marked `is_current` — the code the node runs *now* — as a 0-or-1-element list, ignoring `limit`; revert-safe where the default newest-first ordering is not, since `is_current` (not position 0) is what actually tracks a revert. Returns `NOT_FOUND` for an unknown `unique_id`; a known node with no recorded history (or, under `current_only`, no `:CURRENT` version) returns an empty `versions` list. |
...
| `GetUpstreamChanges` | Returns the node's transitive ancestors' most recent code change, most-recently-changed first, ranked by each ancestor's **effective** last-change time (see below — this is what makes a revert report correctly). Capped at the 5 most-recently-changed ancestors, and each diff independently capped at 8 KiB with `truncated` set on the diffs that were cut — both caps are contract (prompt-size stability for consumers), not tuning; the ancestor cap is applied in Neo4j before any version body is fetched, so a wide DAG cannot pull tens of MiB of code just to discard most of it. `depth` <= 0 defaults to 3 hops server-side; any value above 10 is rejected as `INVALID_ARGUMENT`. `since`, an optional RFC3339 timestamp, excludes ancestors whose effective last-change time predates it. Returns `NOT_FOUND` for an unknown `unique_id`; a node with no changed ancestors (or none surviving the `since` filter) returns an empty `changes` list. |
| `GetCodeUnitVersions` | Returns a shared-code unit's version chain, newest first. Exactly one of `unit_id`/`unique_id` must be set: `unit_id` queries one unit's chain directly; `unique_id` resolves the node's current units first (via its `:CURRENT` `:NodeVersion`'s `:USES_CODE` edges) and, in one batched round trip, returns each of their chains concatenated in resolution order. `limit` <= 0 defaults to 20, clamped to a max of 200. Returns `NOT_FOUND` for an unknown `unit_id` or an unknown node `unique_id`; a known unit with no recorded history, or a known node whose current version uses no shared code, returns an empty `versions` list. |
| `GetNodeRunHistory` | Returns runs that executed the node, newest first (ordered by `:Run.created_at`), optionally filtered server-side to one `operation` (`""` returns every operation). Status and `content_hash` come from the `:EXECUTES` edge — the code that specific run executed; the rest comes from the `:Run` node. `limit` <= 0 defaults to 20, clamped to a max of 200. Returns `NOT_FOUND` for an unknown `unique_id`; a known node with no matching runs returns an empty `runs` list. |
| `GetPrecedents` | Returns past rejections matching an error `signature` — or, when `signature` is empty, a `(category, reason)` pair — resolved-first, then newest. Each entry carries the classified failure, the own-timeline version that resolved it (when one exists) with a server-rendered diff against the version it superseded (8 KiB cap, `truncated` flag), any fix-PR proposals (with each proposal's live `pr_state`), and — for a rejection a merged PR resolved — `edited`, one entry per node the PR touched (which may be an upstream ancestor of the rejected node), each carrying its own diff. `limit` <= 0 defaults to 5, clamped to a max of 20, applied before any code body is fetched. `include_code=false` keeps the failing and resolving code bodies off the wire (the diff is always present). No match is an empty list, never `NOT_FOUND` — a signature is a search key, and "no precedent" is a valid answer. Omitting both `signature` and a complete `(category, reason)` pair is `INVALID_ARGUMENT`. |
...
- **Chain walk starts at `:CURRENT`, but only to compute `is_current`.** The version rows themselves are matched by `unique_id` directly against `:NodeVersion` (the `node_version_uid` index backs this lookup) — a node's versions carry no chain relationship to walk between them. So a retired node — its `:Table` deleted by topology-swap orphan cleanup, its versions still present — still returns its full history; every row just carries `is_current = false` since there is no `:Table` to point a `:CURRENT` edge from.
- **`GetNodeVersions` orders by a version's own `promoted_at`, never `version_seq`.** `promoted_at` is when the release that *first* introduced that exact code was promoted; because a version node is immutable, this stamp never moves even when a later revert re-points `:CURRENT` back at it. `version_seq` is assigned `max+1` at ingestion time, so a late-arriving older release gets the *highest* seq — it is a stable per-node addressing handle (what `GetNodeVersionDiff`'s `from_seq`/`to_seq` address), not a chronological ordering.
- **`GetUpstreamChanges` ranks, filters, and reports on each ancestor's EFFECTIVE last-change time, not a version's own `promoted_at`.** Effective last-change = `max(the ancestor's newest :NodeVersion.promoted_at, its :CURRENT edge's own promoted_at)`. A revert re-points `:CURRENT` at an already-recorded, immutable version without changing that version's `promoted_at`; only the `:CURRENT` edge (stamped `{promoted_at, release_id}` by the write path — `code_version_repository.go`) records when the revert itself happened. Using a version's own timestamp for ranking/`since`/reporting would rank a revert by its stale original creation date and could miss it under a `since` filter entirely. The reported diff's `To` is the version `:CURRENT` currently points to; `From` is the newest-by-`promoted_at` version other than that one — so a revert reports the actual B→A transition, not the two versions' original creation order.
- **`GetUpstreamChanges`'s ancestor cap is enforced before any version body is fetched.** The adapter ranks and `since`-filters ancestor ids using only cheap scalar properties (`promoted_at` values, no `raw_code`/`compiled_code`), applies `LIMIT 5` in that same query, and only then issues a second query that fetches the two relevant versions' full bodies for the retained ids. This bounds the read to a fixed-size response regardless of how many ancestors actually changed.
- **The 5-ancestor and 8 KiB-per-diff caps on `GetUpstreamChanges` are contract, not tuning.** They bound the response size the same way regardless of caller. agent-remediation's validation fixer is one consumer and embeds the result directly into its Step-1 prompt (see `services/agent-remediation.md`) without any further client-side truncation; the `continuo` CLI's `node upstream-changes` command is another, surfacing the same capped diffs to an operator or the LLM chat. Size against both caps together, not just the top-level result count.
- **`GetNodeVersions.current_only=true` follows `:CURRENT` directly instead of walking the chain**, matching a node by `unique_id` on the `:Table→:CURRENT→:NodeVersion` edge rather than the `unique_id`-indexed `:NodeVersion` scan the newest-first path uses, and returns at most one row — `limit` has no effect on it. It additionally requires the matched `:Table` to be active (`COALESCE(t.active, true)`): retirement (`release_promotion_repository.go`) sets `active=false` on a table a newer release dropped while a `:Run` still references it, without removing its `:CURRENT` edge, so an unfiltered match would report that obsolete version as the code the node runs now. A retired node is therefore known but reports no current version — an empty list, not `NOT_FOUND` — distinct from the hard-orphan case the bullet above describes (`:Table` deleted entirely, every version row `is_current=false`). This is the shape the agent-remediation fixer needs as its diff baseline: the code that runs *now*, not whatever sorts first by `promoted_at`, since a revert keeps the reverted-to version's own `promoted_at` at its original (older) value.
- **`GetNodeVersions.include_code=false` keeps `raw_code`/`compiled_code` off the wire between Neo4j and the orchestrator, not merely blank in the response** — the Cypher projection itself substitutes `''` for those two columns rather than fetching and discarding them, so a "light" request cannot approach the CLI's configured 32 MiB gRPC receive limit the way an unbounded `include_code=true` response over the default 20-version page could.
- **Absence of history is a degraded answer, not an error.** A known node/unit with no recorded versions, an unknown-ancestor-changes case, or a node with no matching runs all return an empty list with an OK status. Only an *unknown* `unique_id`/`unit_id` — no `:Table`/`:NodeVersion` (nodes) or `:CodeUnit`/`:CodeUnitVersion` (units) at all — is `NOT_FOUND`. `GetCodeUnitVersions` and `GetNodeRunHistory` apply this the same way `GetNodeVersions` always has: `GetNodeRunHistory`'s `RunExecutions` falls back to the identical known/unknown check on an empty result, and `GetCodeUnitVersions`'s unit-selector path falls back to it under `domain.ErrUnitNotFound` while its node-selector path uses `NodeVersions` as a pure existence probe (the same pattern `GetUpstreamChanges` uses) when `UnitsForNode` comes back empty.
...
- **`GetCodeUnitVersions`'s node-selector path resolves all of a node's units in one batched round trip** (`UnitVersionsBatch`, `UNWIND` over the resolved unit ids), not one `UnitVersions` call — and one new Neo4j session — per unit; a package-heavy model's shared-code closure can run to hundreds of units.

### Failure-precedent case-base read paths

`GetPrecedents` (`orchestrator/service/queries/precedent_query_service.go`, backed by `orchestrator/adapters/neo4j/precedent_query_repository.go`) reads the failure-precedent case base described above.

- **Multi-query pattern, code fetched only for survivors.** An identity query first matches every `:Rejection` reachable from the requested `:ErrorSignature` — by `signature`, or by `(category, reason)` filtered on the `:Rejection`'s own properties when `signature` is empty (the hub's `category`/`reason` are only first-seen metadata, see above) — ranks resolved-first then `at` DESC, and applies `LIMIT` — all on scalar properties, no code bodies touched. A rejection counts as resolved for this ranking whether the `[:RESOLVED_BY]` edge targets a `:NodeVersion` (own-timeline fix) or a `:Proposal` (merged-PR fix). Only the surviving `(release_id, node_id)` keys feed a second, batched detail query that fetches each rejection's fields, its resolving and prior `:NodeVersion` (when resolved on its own timeline), and its `:Proposal`s (with each proposal's live `pr_state`, read off its linked `:PullRequest`, falling back to the `:Proposal`'s own legacy inline snapshot for rows written before the per-service PR split) — the `:PullRequest` join is scoped to the `[:PROPOSED]` edge's `service` (`pl.service = prop.service OR prop.service IS NULL`) so a rejection surfaces only its own service's PR facts, never a sibling service's when the proposal spans several; a third, identically-keyed query then walks each proposal-resolved rejection's `[:RESOLVED_BY]->(:Proposal)-[:EDITED]->(:Table)` edges for its edited-node provenance. A large case base never pays to load and discard code for rows the limit already excluded.
- **A rejection resolves to exactly one own-timeline version, chosen deterministically even when two `[:RESOLVED_BY]->(:NodeVersion)` edges exist.** The forward-link and back-link each guard on "no existing `[:RESOLVED_BY]->(:NodeVersion)` edge", but a race between the two writers can still leave a rejection with two such edges to two different versions — a separate, independently-guarded `[:RESOLVED_BY]->(:Proposal)` edge from a merged fix PR may coexist with either. Both queries pick the same version deterministically: the identity query counts `:NodeVersion`/`:Proposal` targets together (`count(res) > 0`, `res:NodeVersion OR res:Proposal`) so a race never inflates a rejection's rank or duplicates its row; the detail query restricts to `:NodeVersion` targets, orders candidates by `promoted_at` ASC then `content_hash` ASC, and takes the first — the oldest resolving version, matching the back-link's own "oldest version that could have fixed it" rule — before projecting anything from it.
- **`PriorVersion` is the version the fix superseded**, found by matching the resolving version's `NodeVersion {unique_id}` set for `promoted_at` earlier than a baseline and taking the newest. The baseline is the `[:RESOLVED_BY]` edge's own `promoted_at` when present (stamped by the forward-link at resolution time), falling back to the resolving version's own `promoted_at` for edges the back-link wrote (which carries no promotion time). Using the resolving version's own `promoted_at` unconditionally would compute a stale baseline whenever the resolution is a revert, since a reverted-to version node keeps its original, earlier `promoted_at` forever. The resolution diff the query service renders is `PriorVersion → ResolvingVersion`, addressed by each version's `version_seq` — the same stable per-node handle `GetNodeVersionDiff` uses, not a chronological order.
- **Edited-node provenance surfaces the `edited` field, scoped to the resolving service.** For a rejection resolved by a merged fix PR — a `[:RESOLVED_BY {service}]->(:Proposal)` edge — the third query returns one `EditedView` per `[:EDITED]` edge the merged `:Proposal` drew **for that rejection's resolving service**. A batched proposal spanning several services is shared by all their rejections, so the walk is constrained to `ed.service = rb.service` (and the merged `:PullRequest` whose `closed_at` feeds the amended-straddle to `pl.service = rb.service`): a `core` rejection surfaces only `core`'s edits and reads only `core`'s PR close time, never a sibling service's. A legacy edge with a null `rb.service` falls back to the whole-proposal walk (`OR rb.service IS NULL`), so pre-stamp single-service rows still render. The edited node need not be the rejected node itself: a batched fix can resolve a downstream rejection by editing an upstream ancestor, so each entry's `node_id` names whichever `:Table` the PR actually touched. For an edit a human amended before merge, the query additionally resolves the `:NodeVersion`s straddling the PR's close (the first version promoted after the `:PullRequest`'s `closed_at`, and the one immediately before it), so the service renders the merged-truth diff — the code that actually shipped — instead of the proposal's own pre-merge diff; an edit that was not amended keeps the proposal's own stored diff. The query service sets each `EditedView`'s `DiffIsShipped` true only on that merged-truth branch (a straddling version was found), and `precedentToProto` carries it onto the wire as `PrecedentEdit.diff_is_shipped`: an amended edit whose merged version has not been promoted yet falls back to the proposal-time diff with `diff_is_shipped=false`, so a consumer never presents a proposal-time diff as what shipped.
- **`Resolved` folds in proposal-only resolution.** A precedent is `Resolved` when it has an own-timeline resolving `:NodeVersion`, when the merged PR drew at least one `[:EDITED]` edge, or — even with no `[:EDITED]` edges, because every edit's target `:Table` was absent from the graph when the PR merged — when the `[:RESOLVED_BY]->(:Proposal)` edge itself exists (`ResolvedByProposal`, read via an `exists {}` subquery on the detail row). This keeps a genuinely-fixed rejection from rendering as unresolved just because its edit targets never reached the topology.
- **`include_code` governs only the failing rejection's `raw_code`.** The resolving and prior versions' bodies are always fetched from Neo4j (the diff is rendered from them server-side); the query service strips `RawCode`/`CompiledCode` off the *response's* `resolving_version` when `include_code=false`, so the wire-level omission and the diff's presence are independent.
- **`limit` clamps the same way `GetNodeVersions` does**: <= 0 defaults to 5 (deliberately smaller than the code-version paths', since precedents feed LLM prompts), clamped to a max of 20.
...
bounded by it: (1) stop intake by cancelling the root context so consumers
and the outbox processor stop reading new work and unwind their in-flight
handler — the handler's next database call fails against the cancelled
...
registered shutdown handlers (gRPC/HTTP servers, Neo4j, Postgres, Redis)
against a fresh live context with its own deadline fixed at the moment
`Shutdown` starts, while step 2 is capped independently at half the grace
budget; together they leave infra teardown at least the other half of the
budget, live, derived from `context.Background()`, never the
just-cancelled root context. `main` blocks on
...
| `GetObject` | Reads the same code-bundle document, once per rejected release rather than once per node, on the case-base rejections consumer group (`RemediationRequestedRejectionsHandler`, driven by `remediation.requested:v2`'s top-level `code_bundle_uri`) — one message carries the release's whole failing set, and every node's code is read out of that single fetch. An object absent from storage but possibly still landing is likewise left in the PEL for redelivery; a malformed or oversized bundle, or a bundle with no entry for a failing node, records the rejection without its code rather than retrying — losing the code must never cost the precedent. |
...
2. `ReleasePromotionRepository.PromoteRelease` performs the atomic Neo4j topology swap in a single Neo4j transaction (retire-then-orphan-cleanup; see "Topology swap pattern" below). Every upserted node has its `content_hash` refreshed from the event, changed or not — the code-version path compares that property against each release's code bundle, so a stale value would read as a permanent gap. The event's top-level `repo`/`commit_sha`/`promoted_at` provenance is not stamped onto `:Table`; it is consumed only by the separate version-ingestion handler below, which records it on the `:NodeVersion`s it writes. It short-circuits (`changed=false`) when the `:Meta {key:'current_release'}` singleton already records this `release_id`.
...
2. An empty `code_bundle_uri` — a release promoted before topology-controller wrote bundles — is logged and acknowledged: no retry can produce a bundle that was never written.
...
```
(:Table {content_hash})──[:CURRENT {promoted_at, release_id}]──▶(:NodeVersion)
(:NodeVersion)──[:USES_CODE]──▶(:CodeUnitVersion)
(:CodeUnit {unit_id})──[:CURRENT {promoted_at, release_id}]──▶(:CodeUnitVersion)
```

`:NodeVersion` carries `unique_id`, the four hashes (`content_hash`, `source_hash`, `shared_code_hash`, `config_hash`), `runtime`, `raw_code`, `compiled_code`, `compiled_truncated`, `config_json`, the provenance stamp (`repo`, `commit_sha`, `release_id`, `promoted_at`), a per-node monotonic `version_seq`, and the `healed` / `backfilled` provenance qualifiers. `:CodeUnitVersion` carries `unit_id`, `checksum`, `source`, and the same provenance stamp; its `unit_id` is service-namespaced (`<service>:<unit id>`) exactly as the bundle emits it, so two services' copies of a same-named dbt macro never collide. A node's versions carry no relationship between them; each is a free-standing `:NodeVersion` addressed only by `(unique_id, content_hash)` (or, individually, by `version_seq`), and enumerated by matching `unique_id` directly rather than by walking any edge.

Two invariants hold the model together:

- **A version is immutable once written.** Every property is set under `ON CREATE`. A node that reverts to earlier code re-points `:CURRENT` at the version that already exists rather than writing a new one — the graph holds exactly one version per distinct `(unique_id, content_hash)` ever observed. The intermediate version stays reachable by `unique_id`, which the `node_version_uid` index backs.
- **Ordering is decided in the database, never from a pre-write read.** Each node carries a `code_version_promoted_at` watermark; a release advances it for every node it mentions, then guards its pointer write on it inside the same statement. Two things follow. A release that finds the code unchanged still moves the watermark, so a delayed intermediate release cannot later satisfy a stale guard and drag `:CURRENT` backwards. And two replicas promoting different releases serialise on the watermark's node write lock, so an older promotion can never delete a newer pointer and install a stale one.
...
Writes are batched (100 nodes per explicit transaction), and each batch reads the graph's state for its nodes, writes the shared-code versions the changed nodes reference, then the node versions and their `:USES_CODE` edges, and finally the pointer moves and the rejection back-links described below.

#### Failure-precedent case base — graph model

```
(:Table)──[:FAILED {release_id}]──▶(:Rejection)──[:HAS_SIGNATURE]──▶(:ErrorSignature)
(:Rejection)──[:PROPOSED {service}]──▶(:Proposal)──[:HAS_PR]──▶(:PullRequest)
(:Rejection)──[:RESOLVED_BY]──▶(:NodeVersion)
(:Rejection)──[:RESOLVED_BY {amended, service}]──▶(:Proposal)──[:EDITED {path, amended, diff, service}]──▶(:Table)
```

Every classified release rejection the case-base consumers see becomes a `:Rejection`, keyed on `(release_id, node_id)`. `:ErrorSignature` is a global hub, keyed on `signature` alone (not scoped to a release or node), so every past rejection sharing a signature — or the coarser `(category, reason)` pair — is reachable from one node; this is what `GetPrecedents` matches against. `[:FAILED {release_id}]` from the node's `:Table` is written only when that `:Table` exists, so a rejection for a node that never reached the topology (or has since been retired and orphan-deleted) still stands, just unanchored.

`[:RESOLVED_BY]->(:NodeVersion)` converges from both directions of the pipeline, each writer guarded on "no existing `[:RESOLVED_BY]->(:NodeVersion)` edge for this rejection" so whichever arrives first wins and the second is a no-op. The guard is scoped to that one target-label family: it never suppresses, and is never suppressed by, the independently-guarded `[:RESOLVED_BY]->(:Proposal)` provenance edge a merged fix PR draws (see below), so a rejection can carry both an own-timeline version link and a merged-PR provenance link at once.

- The **versions consumer** (`ReleasePromotedVersionsHandler`) forward-links every still-open rejection of a node to the version that just became its `:CURRENT`, the moment a new version is promoted — the ordinary case, where the fix lands after the rejection was recorded. It stamps the edge `ON CREATE` with that promotion's own `promoted_at`/`release_id`, which the detail read path uses as its diff baseline (see above).
- The **rejections consumer** (`RemediationRequestedRejectionsHandler`) back-links a newly-recorded rejection when the fix already landed before the rejections consumer caught up, in two cases. Ordinarily, to the oldest `:NodeVersion` whose own `promoted_at` is after the rejection. For a **revert** — a promotion that reuses an existing, older version node rather than writing a new one, so the version's own `promoted_at` stays at its original, earlier value and the ordinary case finds nothing — it links instead to the node's `:CURRENT` version, detected through the `:CURRENT` edge's own `promoted_at` (always overwritten by the version writer on every promotion) being after the rejection while the version node's own is not. A revert followed by a further promotion can still lose attribution to the version that actually reintroduced the fix; recovering full promotion history is a known gap, not attempted here.

A `:Rejection` may exist as a **stub** (`stub: true`, only `at`/`release_id`/`node_id` set) when a `:Proposal` arrives before its rejection: `PrOpenedProposalsHandler` MERGEs a placeholder so the `[:PROPOSED]` edge has an anchor, and `RemediationRequestedRejectionsHandler` later completes it (`stub: false`) with the classification, signature, and code fields when the rejection itself lands.

When a fix PR reaches its terminal outcome, `PrClosedProvenanceHandler` records the case-base provenance of the resolution. On a **merged** PR it draws, per resolved node, `(:Rejection)-[:RESOLVED_BY {amended, service}]->(:Proposal)` (MERGEing a stub `:Rejection` when one does not yet exist, exactly as the proposal path does), and, per file the PR edited, `(:Proposal)-[:EDITED {path, amended, diff, service}]->(:Table)` — skipping the `[:EDITED]` edge when the edit's target `:Table` is absent, never minting one. `amended` on `[:RESOLVED_BY]` is the OR of the PR's per-edit amend flags: whether a human changed any edit before merge; `service` is the resolving service, which scopes a precedent read of a proposal spanning services to only that service's edits and PR close time (see the read paths above). On a **rejected** PR (closed without merge) it updates only the `:PullRequest`'s terminal `pr_state`/`closed_at` and draws no `[:RESOLVED_BY]`/`[:EDITED]` edges. All writes are idempotent MERGE/SET, so a redelivery converges. This `[:RESOLVED_BY]->(:Proposal)` edge shares the `RESOLVED_BY` type with the version-resolution edge above but targets a different label; the version/rejection writers' "no existing edge" guard is scoped to the `:NodeVersion` target family (see above), so this provenance writer's own MERGE runs independently of theirs — a rejection can carry an own-timeline `[:RESOLVED_BY]->(:NodeVersion)` edge, a merged-PR `[:RESOLVED_BY]->(:Proposal)` edge, or both, without either suppressing the other.
...
4. On success, orchestrator outbox writes (same tx):
   ...
   - 1× `query.model:v1` (carrying `operation`) for the single target node — consumed by `execution-controller` to launch the K8s job, running `dbt test --select <node>` when `operation="test"` or `dbt build --select <node>` when `operation="build"`, in place of `dbt run`/`seed`/`snapshot`.
...
### On `remediation.requested:v2` — RemediationRequestedRejectionsHandler

Consumes `remediation.requested:v2` on the case-base group (`orchestrator-remediation-requested-rejections`, independent of `agent-remediation`'s own group on the same stream). One message carries a rejected release's whole healable failing set, so the handler loops the payload's `nodes[]` and records **one `:Rejection` per node**. No outbox entry is produced.

1. Dedup on `message_processing`, scoped by the consumer-group name.
2. Validates the release-level fields — `release_id` and a non-empty `nodes` array are required, and every node must carry `node_id` and `error_signature` (any violation is a permanent error, so one malformed node drops the whole message rather than half-recording it) — plus `classified_at` (RFC3339, permanent error if unparseable).
3. Resolves the failing candidates' code **once for the whole batch**, from the message's single top-level `code_bundle_uri`: empty (a compile-stage failure, which precedes the parse that produces a bundle) records every rejection with no code and no retry; a bundle absent from object storage but possibly still landing is a plain error, left in the PEL for redelivery; a bundle that fetches but is malformed, oversized, or belongs to a different release than `release_id` (the URI resolved to another release's bundle — unlike `ReleasePromotedVersionsHandler`, which drops the message in this case because writing the wrong code would corrupt the version graph, this handler is code-optional by design) is logged and every rejection is still recorded without code — losing the code must never cost the precedent. A bundle that is readable but holds no entry for one particular node costs only that node its code.
4. For each node, `CaseBaseRepository.RecordRejection` upserts the `:Rejection` (completing a proposal-created stub if one exists), MERGEs its `:ErrorSignature` hub and `[:HAS_SIGNATURE]` edge, anchors `[:FAILED {release_id}]` from the node's `:Table` when one exists, and back-links `[:RESOLVED_BY]` when no `[:RESOLVED_BY]` exists yet — to the oldest version already promoted after the rejection, or, for a revert, to the node's `:CURRENT` version — see "Failure-precedent case base — graph model" above. Every node's write and the dedup claim share the message's single transaction, so a batch is recorded whole or not at all.

### On `remediation.pr_opened:v1` — PrOpenedProposalsHandler
...
3. One PR fixes a whole failing set in one service, so the handler records a proposal for **every node in `resolved_node_ids`** — falling back to the single `node_id` when the payload carries no resolved set — all sharing the same PR facts. For each node, `CaseBaseRepository.RecordProposal` MERGEs a `:Rejection` stub for `(release_id, node_id)` when one does not already exist (`stub: true`, `at` = this PR's `opened_at` as an approximation until the real rejection lands and corrects it), MERGEs the `:Proposal` on `proposal_id`, links it `[:PROPOSED]` from that rejection, and MERGEs the `:PullRequest` on (`proposal_id`, `service`) linked `[:HAS_PR]` from the `:Proposal`. On create it sets all open facts (`pr_state: "open"`, `pr_url`, `pr_number`, `opened_by`, `opened_at`); on match — a replay, or a close that was consumed first — it fills `opened_by`/`opened_at` and backfills `pr_url`/`pr_number` only while still blank, but never touches `pr_state`, so opening a PR whose close already landed does not reset its terminal state to `open`. The `:Proposal` is therefore a single node with one `[:PROPOSED]` edge per node it fixes and one `[:HAS_PR]` edge to its PR facts, so precedent for every fixed node reports the same fix PR.

### On `remediation.pr_closed:v1` — PrClosedProvenanceHandler

Consumes `remediation.pr_closed:v1` on the case-base group (`orchestrator-remediation-pr-closed-provenance`) and records a fix PR's terminal outcome — the resolution provenance of the case base. No outbox entry is produced.

1. Dedup on `message_processing`, scoped by the consumer-group name.
2. Parses `proposal_id`, `release_id` (both required, permanent error if missing), `service`, `pr_url`, `pr_number`, `outcome` (`merged` or `rejected`), `resolved_node_ids`, `edits[]` (`path`, `target_node_id`, `amended`, `diff`), and `closed_at` (RFC3339, permanent error if unparseable). The `resolved_node_ids` and `edits` are passed through verbatim with **no `node_id` fallback**, so a legacy payload that carries neither still updates the PR's terminal state and draws no provenance edges.
3. `CaseBaseRepository.RecordPullRequestOutcome` runs one Neo4j write. It always MERGEs the `:Proposal` and its `[:HAS_PR]->(:PullRequest)` on (`proposal_id`, `service`) and sets `pr_state`/`closed_at` — matching the same `:PullRequest` `PrOpenedProposalsHandler` opened, never a second. Because the open and close events are consumed on independent groups, this write ON-CREATE-fills the `pr_url`/`pr_number` the close event also carries, so a `:PullRequest` created close-first is not blank until the open event's later ON MATCH fills `opened_by`/`opened_at`. Then, **only for a `merged` outcome**, it draws `[:RESOLVED_BY {amended, service}]` from each resolved node's `:Rejection` (MERGEing a stub when absent) to the shared `:Proposal`, with `amended` the OR of the PR's per-edit amend flags and `service` the resolving service (which scopes the precedent read, see above), and `[:EDITED {path, amended, diff, service}]` from that `:Proposal` to each edit's `:Table` — the `[:EDITED]` edge is skipped when its target `:Table` is absent, so this writer never mints a `:Table`. A `rejected` outcome passes empty resolved/edits and so updates only the terminal state. All writes are MERGE/SET, so a redelivery converges.
...
| Redis consumer (`remediation.requested:v2` rejections) | Reads `remediation.requested:v2` on consumer group `orchestrator-remediation-requested-rejections`; dispatches to `RemediationRequestedRejectionsHandler` |
| Redis consumer (`remediation.pr_opened:v1` proposals) | Reads `remediation.pr_opened:v1` on consumer group `orchestrator-remediation-pr-opened-proposals`; dispatches to `PrOpenedProposalsHandler` |
| Redis consumer (`remediation.pr_closed:v1` provenance) | Reads `remediation.pr_closed:v1` on consumer group `orchestrator-remediation-pr-closed-provenance`; dispatches to `PrClosedProvenanceHandler` |
...
|---|---|
| `ui` | `GetScheduleGraph`, `ListRuns`, `GetRunGraph`, `ListScheduleTopologies`, `GetNode` |
...
2. **Upsert** — `MERGE (existing:Table {unique_id: ...}) SET ..., active = true, retired_at = NULL`. Reactivates any node that's back in the new topology. Every upserted node has its properties (including `content_hash`) refreshed unconditionally, changed or not — the per-node `changed` flag drives seed-build triggering and version-write `healed` bookkeeping elsewhere, but no `:Table` write branches on it.
