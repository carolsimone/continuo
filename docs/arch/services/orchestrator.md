...
Consumes `remediation.requested:v1` on the case-base group (`orchestrator-remediation-requested-rejections`, independent of `remediation-agent`'s own group on the same stream) and records each classified failure as a `:Rejection`. No outbox entry is produced.
