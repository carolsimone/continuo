...
| `remediation.pr_closed:v1` | `orchestrator-remediation-pr-closed-provenance` | `PrClosedProvenanceHandler` — stamps a fix PR's terminal `pr_state`/`closed_at` on its `:PullRequest`, and on a **merged** outcome draws the case-base provenance edges: `[:RESOLVED_BY {amended}]` from every resolved node's `:Rejection` to the shared `:Proposal`, and `[:EDITED {path, amended, diff, service}]` from that `:Proposal` to each edit's `:Table` (skipped when the `:Table` is absent). A **rejected** outcome updates only the terminal state. No outbox emission |
...
(:Rejection)──[:RESOLVED_BY {amended}]──▶(:Proposal)──[:EDITED {path, amended, diff, service}]──▶(:Table)
```
...
1. Dedup on `message_processing`, scoped by the consumer-group name.
2. Parses `proposal_id`, `release_id` (both required, permanent error if missing), `service`, `outcome` (`merged` or `rejected`), `resolved_node_ids`, `edits[]` (`path`, `target_node_id`, `amended`, `diff`), and `closed_at` (RFC3339, permanent error if unparseable). The `resolved_node_ids` and `edits` are passed through verbatim with **no `node_id` fallback**, so a legacy payload that carries neither still updates the PR's terminal state and draws no provenance edges.
3. `CaseBaseRepository.RecordPullRequestOutcome` runs one Neo4j write. It always MERGEs the `:Proposal` and its `[:HAS_PR]->(:PullRequest)` on (`proposal_id`, `service`) and sets `pr_state`/`closed_at` — matching the same `:PullRequest` `PrOpenedProposalsHandler` opened, never a second. Then, **only for a `merged` outcome**, it draws `[:RESOLVED_BY {amended}]` from each resolved node's `:Rejection` (MERGEing a stub when absent) to the shared `:Proposal`, with `amended` the OR of the PR's per-edit amend flags, and `[:EDITED {path, amended, diff, service}]` from that `:Proposal` to each edit's `:Table` — the `[:EDITED]` edge is skipped when its target `:Table` is absent, so this writer never mints a `:Table`. A `rejected` outcome passes empty resolved/edits and so updates only the terminal state. All writes are MERGE/SET, so a redelivery converges.
