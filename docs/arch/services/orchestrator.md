...
| `ErrorSignature` node | Global precedent hub keyed by `signature`; carries `category` and `reason`, set once on create. Every `:Rejection` sharing a signature links to the same hub, which is what makes a signature (or a `(category, reason)` pair) a cross-release, cross-node lookup key. |
...
| `RESOLVED_BY` relationship | `(:Rejection)-[:RESOLVED_BY]->(:NodeVersion)` — links a rejection to the version that resolved it. Two consumers converge on this edge from opposite directions, each guarded on "no existing `RESOLVED_BY`" so whichever reaches an open rejection first wins: the rejections consumer back-links to the oldest already-promoted version newer than the rejection, when the fix landed before the rejection was recorded; the versions consumer forward-links every still-open rejection of a node the moment a new version becomes that node's `CURRENT`. |
...
| `error_signature_category_reason` | index, `:ErrorSignature(category, reason)` | `GetPrecedents`' fallback lookup when no signature is given. |
...
- **Two-query pattern.** An identity query first matches every `:Rejection` reachable from the requested `:ErrorSignature` (by `signature`, or by `(category, reason)` when `signature` is empty), ranks resolved-first then `at` DESC, and applies `LIMIT` — all on scalar properties, no code bodies touched. Only the surviving `(release_id, node_id)` keys feed a second, batched detail query that fetches each rejection's fields, its resolving and prior `:NodeVersion` (when resolved), and its `:Proposal`s. A large case base never pays to load and discard code for rows the limit already excluded.
- **`PriorVersion` is the version the fix superseded**, found by matching the resolving version's `NodeVersion {unique_id}` set for `promoted_at` earlier than the resolving version's own `promoted_at` and taking the newest. The resolution diff the query service renders is `PriorVersion → ResolvingVersion`, addressed by each version's `version_seq` — the same stable per-node handle `GetNodeVersionDiff` uses, not a chronological order.
...
- The **versions consumer** (`ReleasePromotedVersionsHandler`) forward-links every still-open rejection of a node to the version that just became its `:CURRENT`, the moment a new version is promoted — the ordinary case, where the fix lands after the rejection was recorded.
- The **rejections consumer** (`RemediationRequestedRejectionsHandler`) back-links a newly-recorded rejection to the oldest `:NodeVersion` already promoted after it, when that fix landed before the rejections consumer caught up to the failure that motivated it.
