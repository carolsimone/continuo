...
- **Two-query pattern.** An identity query first matches every `:Rejection` reachable from the requested `:ErrorSignature` — by `signature`, or by `(category, reason)` filtered on the `:Rejection`'s own properties when `signature` is empty (the hub's `category`/`reason` are only first-seen metadata, see above) — ranks resolved-first then `at` DESC, and applies `LIMIT` — all on scalar properties, no code bodies touched. Only the surviving `(release_id, node_id)` keys feed a second, batched detail query that fetches each rejection's fields, its resolving and prior `:NodeVersion` (when resolved), and its `:Proposal`s. A large case base never pays to load and discard code for rows the limit already excluded.
- **A rejection resolves to exactly one version, chosen deterministically even when two `[:RESOLVED_BY]` edges exist.** The forward-link and back-link each guard on "no existing edge", but a race between the two writers can still leave a rejection with two edges to two different versions. Both queries pick the same one: the identity query counts edges only (`count(res) > 0`) so a race never inflates a rejection's rank or duplicates its row; the detail query orders candidates by `promoted_at` ASC then `content_hash` ASC and takes the first — the oldest resolving version, matching the back-link's own "oldest version that could have fixed it" rule — before projecting anything from it.
...
`[:RESOLVED_BY]` converges from both directions of the pipeline, each writer guarded on "no existing `[:RESOLVED_BY]` for this rejection" so whichever arrives first wins and the second is a no-op:
