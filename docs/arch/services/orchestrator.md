...
**Dispatch frontier.** Each PENDING projection row carries `ReadyToDispatch`. A rebased node is on the frontier (`ReadyToDispatch = true`) only when none of its upstreams is itself rebased — equivalently, when it is not a topology descendant of another rebased node; every upstream is inherited-SUCCEEDED or outside the run. `DispatchDerivedRun` emits a `query.model:v1` for the frontier rows only. Blocked rebased rows wait for the run aggregate: as a frontier node completes, `CompleteNode` emits `NodeUnblocked` for newly-ready downstream nodes (→ `query.model:v1`) or cascade-skips them when the upstream fails. This mirrors the fresh-run roots-only dispatch and ensures a re-pended SKIPPED node does not run until its upstream succeeds — and is skipped again if it re-fails.
