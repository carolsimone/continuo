...
- **The 5-ancestor and 8 KiB-per-diff caps on `GetUpstreamChanges` are contract, not tuning.** They mirror the caps remediation-agent's prompt builder enforces on its GitHub-read upstream diffs (see `services/remediation-agent.md`), so a consumer switching between the two paths sees identical prompt sizes — size against both caps together, not just the top-level result count.
