...
- The third strips the retired `last_commit_sha`/`last_repo`/`last_changed_at`/`last_release_id` provenance stamp from every `:Table`: `MATCH (t:Table) WHERE t.last_commit_sha IS NOT NULL OR t.last_repo IS NOT NULL OR t.last_changed_at IS NOT NULL OR t.last_release_id IS NOT NULL REMOVE t.last_commit_sha, t.last_repo, t.last_changed_at, t.last_release_id`. Version provenance lives on `:NodeVersion`; nothing reads these `:Table` properties any more, and leaving them in place would serve a stale commit to a future reader. `REMOVE` on an absent property is a no-op, so re-running it is idempotent.
