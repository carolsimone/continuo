...
| Entity | Description |
|---|---|
| `Table` node | One per model/seed; carries topology metadata (`schema_name`, `table_name`, `service_name`, `node_type`, `test_count`, `schedule_name`, `image_tag`, `original_file_path`, `dbt_unique_id`, `runtime_manifest_uri`, `runtime_manifest_sha256`, `runtime_manifest_dbt_version`, `runtime_manifest_parse_context_sha256`), `last_updated_at`, an `active` flag for current-topology reconciliation, and per-node provenance properties (`last_commit_sha`, `last_repo`, `last_changed_at`, `last_release_id`) that record the most recent release in which the node's `content_hash` changed. `test_count` is the number of dbt tests declared for the node, carried on `release.promoted:v1` and defaulted to `0` when absent (`COALESCE(t.test_count, 0)` on read). |
...
| `EXECUTES` relationship | Directed edge from `Run` to `Table`; carries per-run `status`, pre-assigned `task_id` UUID, per-task `image_tag` + `manifest_version`, the pinned `dbt_unique_id` + four `runtime_manifest_*` properties copied from the `:Table` at snapshot time, and (for rebase-projected inherited rows only) an optional `inherited_from_task_id` property pointing to the root executed `task_id` in the source lineage |

#### Runtime manifest pinning

`dbt_unique_id` and the four `runtime_manifest_*` properties exist on both the `:Table` and the `:EXECUTES` edge, and the two answer different questions. The `:Table` says what the **current** release targets; the edge says what **this run** executes. The snapshot writer copies `:Table` → `:EXECUTES` once, at `Snapshot` time, and nothing rewrites the edge afterwards.

Once a run is snapshotted, every read on its execution path therefore resolves from the edge: the rehydrate queries behind `NodeUnblocked` read `e.dbt_unique_id` / `e.runtime_manifest_*`, and `LoadSourceTasks` / `LoadSingleTableFromSourceRun` read the source run's edges for rerun and `snapshot_of_run`. Only a fresh run's selectors (`LoadLatestSourceDAG`, `LoadSingleLatestTable`) read the `:Table`, because a fresh run is defining its own pin. `RebasePartition` is the deliberate mix: rebased nodes re-execute and take the current `:Table`, while inherited nodes keep the source edge that produced them.

A promotion rewrites `:Table` properties in place. Re-reading the `:Table` mid-run would dispatch a run's later nodes against a different artifact than its earlier ones — one run's output spliced from two releases, with no error surfaced. Pinning on the edge is what prevents it.

The four `runtime_manifest_*` fields travel as one unit (`pkg/domain/model.RuntimeManifestRef`): all four set, or all four empty. An empty reference is valid and routes the node down the per-node Job path; a partial one is rejected at the `release.promoted:v1` parser as a permanent error. `dbt_unique_id` (dbt's node key, e.g. `model.finance.orders`) is a separate identity from the graph's `unique_id` (`schema.table`) and never substitutes for it.
...
| Stream | Trigger |
|---|---|
| `query.model:v1` | One message per newly-ready downstream node after a SUCCEEDED node is processed; for rerun/rebase only the rebased rows on the **dispatch frontier** get an initial `query.model:v1` (a rebased row whose upstream is itself rebased waits until that upstream completes; inherited rows are already SUCCEEDED at dispatch and never enter the executor pipeline). Carries `operation` (`""`, `"test"`, or `"build"`) on every dispatch path: the initial frontier (`Params.Operation`), the downstream unblock path (`Run.Operation`, read back on rehydrate and stamped onto `NodeUnblocked`), and the derived-run frontier (`DerivedRunDispatch.Operation`, resolved from the source run via `SnapshotService.SourceOperation`) — so `executor-controller` resolves `dbt test`/`dbt build` instead of `dbt run`/`seed`/`snapshot` accordingly, on whichever path dispatched the node. A node pinned to a runtime manifest additionally carries `dbt_unique_id` and the four `runtime_manifest_*` stream fields, taken from the run's `:EXECUTES` edge on each of the four run dispatch paths (initial frontier, downstream unblock, derived-run frontier, single-node run). The publisher adds `dbt_unique_id` only when populated, and the `runtime_manifest_*` fields only when the reference is complete: a node with no pin omits all five, leaving its field map identical to a dispatch produced before runtime manifests existed, and `executor-controller` runs it down the per-node Job path. The seed-build dispatch (`SeedBuildOnPromoteHandler`) snapshots no run and carries no pin, so its seeds always take that path. |
...
The swap stamps each node's `dbt_unique_id` and runtime manifest reference onto its `:Table` alongside the rest of its topology metadata, so the next run to snapshot the graph pins the artifact this release published. `ParseReleasePromoted` validates every node's reference before the swap: a complete or an entirely empty reference is accepted, a partial one is an `events.ErrPermanent` (the consumer ACKs and drops the message, since no retry can complete it).
