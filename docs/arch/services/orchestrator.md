...
| `code_unit_version_unit_id` | index, `:CodeUnitVersion(unit_id)` | The `unit_id`-only lookups `GetCodeUnitVersions` issues (`UnitVersions`, and once per unit — now batched into one round trip, see below — on the node-selector path). `code_unit_version_unique` only covers the composite `(unit_id, checksum)`, so without this index Neo4j planned a full `:CodeUnitVersion` label scan for every one of those reads. |
