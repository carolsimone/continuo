...
| `rationale` | Composed from facts, not the model's text alone (no warehouse data), one block per proposed cluster separated by a blank line: `Failed:` one line per failing node, `This release changed upstream:` lines (nearest first, up to three) or `No upstream of <node> changed in this release.` — the whole section absent for a compile-stage attempt, whose trigger carries no changed-ancestor facts — `Edited:` one line per changed path naming the node it repairs, one line per consumer for a cross-service cluster naming it unable to change in this release with the follow-up of adopting the new shape left to its own service, and last the model's own one-sentence note (describing its edit, not a cause) under a "Model's note:" label. |
