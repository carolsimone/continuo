...
`pr_services` (`Service.PRServices`, `service/proposals/service.go`) is the sorted set of owning-service groups a proposal's pull requests split into, and is what `BeginPullRequest`'s `service` argument must name one of. A proposal whose edits carry no cluster members — one predating the per-service split — is never split: `pr_services` is the single legacy `[""]` group. Once at least one edit attributes members, the groups are `GroupEditsByService(ServiceRepoPaths, edits)`'s keys, one PR per owning service — unless `ServiceRepoPaths` leaves some edit's path unmapped, in which case that edit lands under the same `""` key a legacy proposal uses. Splitting on that grouping as-is would open a `""` PR (the repository's whole-proposal claim: every edit) alongside a named-service PR holding only its own edit, so the mapped edit would be claimed by both. `PRServices` collapses the whole proposal back to the single `[""]` group whenever the grouping contains `""` alongside a named key, so an incomplete `ServiceRepoPaths` degrades to one whole-proposal PR rather than opening overlapping ones.
...
| `remediation.pr_closed:v1` | `orchestrator` (group `orchestrator-remediation-pr-closed-provenance`) | The PR-outcome reconciler observes a terminal GitHub PR state and `RecordOutcome` performs the CAS `open → merged | rejected`; payload is pointer-only: `proposal_id`, `release_id`, `node_id`, `resolved_node_ids` (every failing node the PR actually fixed, sorted — narrowed by `node_outcomes` exactly as `pr_opened` is), `service` (the owning-service group this PR covers; omitted for a legacy whole-proposal PR), `pr_url`, `pr_number`, `outcome` (`merged` or `rejected`), `closed_at` (RFC 3339), and `edits[]` (each `{path, target_node_id, amended, diff}` — whether a human amended that edit before merge, and the amendment diff; empty for a rejected PR). `event_id` is a deterministic SHA1 UUID keyed on `release_id|attempt|service` (the legacy service `""` reproducing the pre-split `release_id|attempt` id byte-for-byte), under a namespace distinct from the `pr_opened` id. Orchestrator's provenance consumer stamps the `:PullRequest`'s terminal state and, on a merged outcome, draws the `[:RESOLVED_BY {amended, service}]`/`[:EDITED {path, amended, diff, service}]` case-base edges. |
...
`parseFixer` (`service/fixer/parse.go`) fixes a node whose compiled SQL topology-controller's parser rejected — invalid SQL, or a relation referenced without its schema qualifier — with exactly one LLM call. It shares `gatherSourceFile` (`service/fixer/gather.go`) with the compile fixer below, so the repo read, the co-located-yml/`dbt_project.yml` best-effort context, and the single-file interpretation are identical; only the repo-prefix lookup key and the prompt differ.

It repairs dbt nodes only. A python node's parse failure is classified and triggers remediation exactly like any other, but this fixer skips it before any read, so the attempt records that node `skipped` and no proposal is produced for it. The reason is that the SQL the parser rejected is not in the file the trigger names: topology-controller reports a python node's parse failure against the script its contract entry declares, while the rejected SQL is one of that entry's `reads` in the service's `contract.yaml` — a file this system carries no repository path for. Editing the script would leave the rejected SQL untouched, and the resulting proposal, packaging no `VerificationContract`, would be submitted as a dbt verification run for a python service.

```
1. node_type is python-model or python-csv: proposal(status=skipped), done —
   no source read, no precedent lookup, no LLM call.
2. Empty file_path or empty service on the trigger: proposal(status=skipped), done.
3. gatherSourceFile(svc, in, in.Service, "parse fix") — the same read
   compileFixer uses below, but keyed on the trigger's `service` field rather
   than `node_id`: a parse failure's node_id is a real node id, not
   compile's synthetic service id, so it cannot itself resolve a repo
   prefix.
...
4. No dbt log is fetched: the rejection happens before any Job runs, so the
...
6. On a proposed outcome, diff the corrected content against the resolved
...
| Validation fixer, python-csv node (shares `locateContractForFix` and `proposeContractFixViaVerification` with the python-model lane) | `agent-remediation/service/fixer/csv_validation.go` |
