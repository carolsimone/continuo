...
15. Build the proposal: confidence, rationale, and model all come from the
    ...
    proved by a shadow release, rather than by the model's guess.
```
...
| Shadow-verify reconciler loop (poll every verification, combine verdicts, CAS finalization, replay the whole batch on a rejection, per-release timeout budget) | `agent-remediation/service/shadowverify/reconciler.go` |
