...
| `rationale` | Composed from facts, not the model's text alone (no warehouse data): for each proposed cluster, what failed, what this release changed upstream (or that nothing did), what was edited, and, when the failure crossed a service boundary, why the edit sits at the ancestor; the model's own one-line note is rendered under a "Model's note:" label. One block per cluster, separated by a blank line. |
