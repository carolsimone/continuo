...
| `FailPullRequest(id)` | Resets `pr_state` from `'opening'` to `'failed'` and clears `pr_claimed_at` back to `NULL`, making the action retryable and ensuring a later re-claim of the same proposal ages from its own (later) `pr_claimed_at`, never this one. Called by ui-service when the GitHub step errors after a successful `BeginPullRequest` claim, and by the reconciler's opening sweep when a claim's age exceeds `REMEDIATION_PR_OPENING_GRACE_PERIOD` with no matching PR on GitHub. |
...
| Opening sweep (same reconciler, same tick) | Recovers `pr_state='opening'` claims left stranded when ui-service's `RecordPullRequest` call fails after the PR was already created on GitHub (that step is explicitly best-effort; see `ui-service/src/server/routes/remediation.ts`), and releases genuinely abandoned claims for retry. Each pass lists up to 50 proposals with `pr_state='opening'`, oldest-created first, each carrying its `pr_claimed_at`; for each row the sweep recomputes the deterministic branch name (`remediation/<release_id>/<node_sanitized>-attempt<n>`, `proposals.BuildBranch`) and calls `GET /repos/{repo}/pulls?head=…` (GitHub Pulls API). A found PR — at any claim age — is recorded via `Service.Record` exactly as the client-side flow would have, closing the gap: `opening → open` with `pr_url`/`pr_number` set and `pr_claimed_at` cleared to `NULL`, so the UI stops inviting a duplicate PR. When no PR is found, the sweep compares `now - pr_claimed_at` (wall-clock, via the reconciler's `Clock` port) directly against `REMEDIATION_PR_OPENING_GRACE_PERIOD`: a claim younger than the grace period is left untouched and retried next pass, while one older than it calls `Service.Fail` (`opening → failed`, `pr_claimed_at` cleared to `NULL`), which ui-service's claim guard (`pr_state IN ('', 'failed')`) accepts as retryable. `pr_claimed_at` is stamped by `BeginPullRequest` from the service's own clock and cleared on every exit from `'opening'` (`RecordPullRequest`, `FailPullRequest`), so a proposal re-claimed after a prior failure (`opening → failed → opening`) always ages from its own, second claim. A row with `pr_claimed_at IS NULL` — the shape of a claim taken before this column existed and not backfilled — is left untouched rather than failed, since an unmeasurable claim can never safely be judged stale. Because age is read from a stored timestamp rather than accumulated across passes, the decision is exact and survives a process restart unchanged; a claim taken moments before a pass runs is never raced, since its age is always far short of the grace period regardless of poll interval. A per-row GitHub error leaves the row untouched — an inconclusive read is not a confirmed miss — and does not block the rest of the batch. |
...
| PR-outcome reconciler loop, incl. permission-gap degraded signal and the opening sweep (recover-or-fail stuck `pr_state='opening'` claims) | `remediation-agent/service/proposals/reconciler.go` |
...
| Postgres UoW + proposal repo (incl. CAS for BeginPR and RecordPROutcome, open-PR listing, stuck-opening listing) | `remediation-agent/adapters/postgres/` |
