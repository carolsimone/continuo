...
**Runtime**: Go service. HTTP on port 8092: `/healthz` (liveness, always 200) and `/reconcile/status` (JSON `{degraded, reason}` reporting whether PR-outcome reconciliation is currently blocked by a token-permission gap; always 200 so the condition surfaces without crash-looping the pod). gRPC `RemediationProposals` server on port 50054. Depends on Postgres (`continuo_remediation_agent`), Redis, S3, the orchestrator gRPC endpoint (`GetNodeAncestry`, port 50052), and GitHub, exclusively via read-only GETs against the Contents API (source file/directory reads) and the Pulls API (PR status polling for the reconciler).
...
| `REMEDIATION_AGENT_HTTP_PORT` | no | `8092` | HTTP port serving `/healthz` and `/reconcile/status` |
...
| Reconciler degraded/health signal (permission-gap tracker) | `remediation-agent/service/proposals/health.go` |
