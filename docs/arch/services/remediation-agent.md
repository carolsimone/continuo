...
| `FailPullRequest(id)` | Resets `pr_state` from `'opening'` to `'failed'`, making the action retryable. Called by ui-service when the GitHub step errors after a successful `BeginPullRequest` claim, and by the reconciler's opening sweep when a claim goes stale with no matching PR on GitHub. |
...
| Opening sweep (same reconciler, same tick) | Recovers `pr_state='opening'` claims left stranded when ui-service's `RecordPullRequest` call fails after the PR was already created on GitHub (that step is explicitly best-effort; see `ui-service/src/server/routes/remediation.ts`), and releases genuinely abandoned claims for retry. Each pass lists up to 50 proposals with `pr_state='opening'`, oldest-created first; for each it recomputes the deterministic branch name (`remediation/<release_id>/<node_sanitized>-attempt<n>`, `proposals.BuildBranch`) and calls `GET /repos/{repo}/pulls?head=…` (GitHub Pulls API). A found PR — at any claim age — is recorded via `Service.Record` exactly as the client-side flow would have, closing the gap: `opening → open` with `pr_url`/`pr_number` set, so the UI stops inviting a duplicate PR. When no PR is found, the proposal row carries no claim timestamp to measure elapsed time against (`created_at` predates the claim by an arbitrary amount — a proposal can sit reviewable long before an operator claims it), so the sweep instead counts consecutive passes that found no PR for that proposal id, in an in-process map rebuilt fresh each pass (a process restart resets the count, which only extends the grace period, never shortens it). Once a claim has missed `REMEDIATION_PR_OPENING_GRACE_PERIOD / REMEDIATION_PR_POLL_INTERVAL` consecutive passes (default 10 passes, i.e. ~10 minutes at the default 60s interval) it calls `Service.Fail` (`opening → failed`), which ui-service's claim guard (`pr_state IN ('', 'failed')`) accepts as retryable. A miss streak resets to zero as soon as the id drops out of the stuck-opening listing (recorded, failed, or otherwise resolved) and starts over if it reappears later. This threshold, not a wall clock, is why a claim taken moments before a pass runs is never raced: it needs a full run of consecutive misses, comfortably longer than the seconds-scale S3-fetch-then-create-PR round trip a healthy claim completes in, before it is ever failed. A per-row GitHub error is logged and carries the prior miss count forward unchanged (an inconclusive read is neither a hit nor a miss) rather than blocking the rest of the batch. |
...
| `REMEDIATION_PR_OPENING_GRACE_PERIOD` | no | `10m` | How long the opening sweep waits, expressed as wall-clock time and converted to a whole number of reconcile passes via `REMEDIATION_PR_POLL_INTERVAL` (minimum 1 pass), before releasing a `pr_state='opening'` claim with no matching GitHub PR back to `'failed'`. A non-positive value falls back to the default. |
