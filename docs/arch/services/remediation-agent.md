...
1. Decode trigger: extract source, release_id, node_id, relation_id,
   error_signature, category, reason, error_excerpt, dbt_log_uri,
...
2. Count prior TERMINAL attempts for (source, node_id, error_signature). In-flight
   'generating' and 'verifying' rows are excluded, so neither the in-progress
   attempt nor one still awaiting a shadow release's verdict inflates the cap or
   shifts the attempt number across a redelivery.
...
| `SHADOW_VERIFY_TIMEOUT` | no | `20m` | How long a proposal awaits its shadow release's verdict, measured from when the attempt was recorded, before the reconciler ends the attempt as failed (Go duration). A non-positive value falls back to the default, so a misconfiguration can never leave a proposal waiting indefinitely. |
| `SHADOW_VERIFY_POLL_INTERVAL` | no | `15s` | Interval between shadow-verify reconciler passes (Go duration). A non-positive value falls back to the default, so a misconfiguration can never produce a hot loop. |
