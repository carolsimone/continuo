...
- `ReleaseDetailPage`: full detail view for a single release at `/releases/:id`. Fetches `/api/releases/:id`. Shows status, bootstrap flag, reject reason (the humanized failed-stage label, matching the `Releases` list), and a per-node validation results table grouped by stage (Compilation/Seed/Validation). Each row with a `dbt_log_uri` includes an inline log viewer that fetches `/api/releases/log?key=<uri>` on demand, and a status-aware FIX cell (see below) that surfaces the "Generating fix…" chip and then the "Proposed fix available →" link for a healable failed node.
- `SchedulerCard`: displays schedule name, running status, cron expression, last run time and progress; polls `/api/schedulers/:last_run_id/tasks` for task progress, walking every page via `fetchAllPages` so the progress total reflects the whole run rather than the first page, and `/api/runs/:last_run_id/graph` for topology-drift information (both every 5 s); shows a warning strip when the last run's `run_topology_generation` is older than the orchestrator's `latest_topology_generation`, matching the drift logic used on the schedule detail page; includes a "Trigger run" button to start a full DAG run (disabled while a run is active) and a "Cancel" button while a run is in flight
