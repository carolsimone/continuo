...
- `ReleasesPanel`: displays the live `current_prod` release, the first in-flight candidate (parsing or validating), and a paginated release history list with status filtering. Fetches `/api/releases/current-prod` and `/api/releases` (with optional `status` and `cursor` params); supports load-more pagination via `next_cursor`. Each release row links to `ReleaseDetailPage`. The history table carries a `Reason` column: a rejected release shows its `reject_reason` as the humanized failed-stage label (Compilation / Seed build / Validation); every other row renders a muted `—`. A release whose `shadow` flag is set — one posted by `remediation-agent` to verify a proposed fix — carries a `verification` chip beside its id, and `validated` (a shadow release's terminal success) appears in the status filter and renders in the same succeeded colour as `promoted`. Shadow releases are never hidden or filtered out by default: an operator has to be able to see what the system is doing on its own initiative, and the chip is what says so.
