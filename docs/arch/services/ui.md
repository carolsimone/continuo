...
ui holds the GitHub App ID, private key, and installation ID (`GITHUB_APP_ID`, `GITHUB_APP_PRIVATE_KEY`, `GITHUB_APP_INSTALLATION_ID`). For each service group in a Create PR request it mints a short-lived (~1h) installation token and performs:
...
- `ReleaseDetailPage`: full detail view for a single release at `/releases/:id`. Fetches `/api/releases/:id` and, while the release status is non-terminal (anything other than `promoted`/`validated`/`rejected`/`superseded`), self-schedules another fetch every 5s so the per-node validation results table — which the backend projects incrementally during `validating` — grows live without a manual refresh; polling stops as soon as a terminal status is observed. Shows status, bootstrap flag, a `fix verification run` chip when the release is a shadow, the status pill suffixed `· round N` once `remediation_round` exceeds 1, and — for a rejected release — an error strip carrying the humanized failed-stage reject reason (matching the `Releases` list) with the persisted `reject_detail` appended after an em dash when the release row carries one (empty for a rejection whose reject path supplied no detail, and for every release row written before the `reject_detail` column existed). Also shows the per-node validation results table grouped by stage (Compilation/Seed/Validation). Each row with a `dbt_log_uri` includes an inline log viewer that fetches `/api/releases/log?key=<uri>` on demand, and a status-aware FIX cell (see below) that surfaces the "Generating fix…" and "Verifying fix…" chips, the "Proposed fix available →" link for a healable failed node, and — once a node's latest attempt has landed on a terminal-but-blank outcome with no fix in flight — a note explaining why it stopped: "Attempt budget spent." for `escalated`, the attempt's `rationale` (or a default) for `failed`, and that rationale plus "Fix it in the repository." for `skipped`. A shadow release's own per-node results and logs are inspectable exactly like any other release's, so the run that decided a fix can be read as easily as the run that failed.
  A rejected, non-shadow release with at least one failed node shows a **Try again (round N of 3)** button once it is a dead end: the current remediation round has produced at least one proposal, none of those proposals has a node still generating/verifying/proposed, and no proposal from any round has a PR out for review. Clicking it calls `POST /api/releases/:id/retry-remediation`; a `202` bumps the displayed round and restarts proposal polling, a `409 rounds_exhausted` at the cap replaces the button with a static "push a new commit" notice, and any other 409/502/500 renders its reason as an `info-strip--error` (`role="alert"`) below the button — including `retry_in_progress` ("A retry is already in progress — wait for the new round to start.") for the window between the round bumping and its first proposal landing, and `proposal_open` naming the PR when release-controller supplies one.
