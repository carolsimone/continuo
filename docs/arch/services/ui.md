...
|---|---|---|---|
| `/api/remediation/proposals` | GET | authenticated | `ListProposals` → agent-remediation gRPC. Returns proposals ordered `created_at DESC`. Supports `status`, `pr_state`, and `service` filter query params for the inbox view; `service` matches a proposal when it is among the proposal's services. |
