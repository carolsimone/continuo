...
- a release detail page: per-node validation results with an inline dbt log viewer, and a Verification runs section linking to every fix-verification run submitted against that release
- a verification-run detail page: a single fix-verification run's status, transitions, and per-node results
