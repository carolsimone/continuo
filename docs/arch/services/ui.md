...
| `/api/remediation/proposals/:id/pull-request` | POST | `operator` | `BeginPullRequest` → claim; S3 read of every edit's `content_uri` from the claim's `edits` list (one entry per changed file, each carrying the `target_node_id` it repairs). **One proposal produces exactly one pull request, however many nodes it fixes**: the title is `[remediation] fix <node> (release <id>)` when the claim's `resolved_node_ids` names one node and `[remediation] fix <n> nodes (release <id>)` when it names several, the body lists every one of them under **Nodes:** and annotates each changed file with the node it fixes, and the branch is `remediation/<release_id>/attempt<n>`. A claim whose `edits` list is empty but whose single-file fields (`file_path`, `proposed_sql_uri`) are populated — what an agent-remediation instance that predates the list sends during a rolling upgrade — is read as one edit synthesized from those fields and proceeds normally; a claim with neither is refused, the claim released via `FailPullRequest` and the route returns 502 rather than opening a pull request with no files. GitHub App: create branch + one tree commit carrying every fetched file + open PR; `RecordPullRequest` on success or `FailPullRequest` on GitHub error. Returns `{ pr_url, pr_number }`. Returns 409 when the proposal is already `opening`/`open` (with the existing `pr_url`) or when `source_resolved=false` — both are `FAILED_PRECONDITION` from agent-remediation and the route maps every `FAILED_PRECONDITION` to 409. Returns 502 when the claim describes no file at all, on a failed S3 read, or on a failed GitHub PR creation. |
