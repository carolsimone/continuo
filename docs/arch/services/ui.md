...
- a **Remediation** tab (5th dashboard tab): lists fix proposals from `agent-remediation` with diff, rationale, confidence, and source lineage. A proposal is one attempt at a whole rejected release, so it names every node it resolves and every file it changes; `operator` users can click **Create PR** to open a single GitHub pull request committing every one of those files in a single commit; `viewer` users see the surface read-only. ui holds the GitHub App write credential, reads each changed file's corrected content from S3, and records the PR result back to agent-remediation over gRPC. Each proposal's `pr_state` is rendered as a colored chip once it reaches a terminal outcome — `merged` or `rejected` — mirrored from GitHub by agent-remediation's PR-outcome reconciler; non-terminal `pr_state` values render as plain text. The release detail page carries a lightweight back-link to any associated proposal.
...
|---|---|---|---|
| `/api/remediation/proposals` | GET | authenticated | `ListProposals` → agent-remediation gRPC. Returns proposals ordered `created_at DESC`. Supports `status` and `pr_state` filter query params for the inbox view. |
