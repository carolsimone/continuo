...
**What counts as changed.** Every run's validation seed set starts from the candidate nodes whose `content_hash` differs from `current_prod` (`changedNodeIDsFor`, `service/handlers/handle_parsed_manifest.go`). A candidate, and a verification naming no `verifies_release_id`, stop there. A verification naming `verifies_release_id` re-measures a node that differs from `current_prod` when the fix touched it (its hash differs from the rejected release's candidate topology, or it is absent from that topology) OR the rejected release did not record it in `failing_nodes` (`release.VerificationSeedSet`). So the fix's own edited nodes are rebuilt; every node the rejected release changed and validated ok is rebuilt from the candidate, which gives its descendants — the fix's node among them — the shape that release produced rather than production's older one; and a node the rejected release recorded as not ok (failed or skipped) that the fix did not touch is left out and cloned from production: a fix spanning two services submits one verification per edited service, each assembling the OTHER edited service's node unchanged and still broken, and that node is measured by its own verification. A node another candidate promoted since the rejection matches `current_prod` and is never a seed. When the verified release is unreadable or has no candidate topology, `changedNodeIDsFor` falls back to the plain `current_prod` diff, the same graceful degradation `assembleFor` takes.
...
status=ok:
  ...
  derive changed = candidate nodes whose content_hash differs from current_prod, or are new
             # for a verification naming verifies_release_id: of the nodes that differ
             # from current_prod, keep those the fix touched (differ from the rejected
             # candidate) or the rejected release did not list in failing_nodes. A
             # node the rejected release changed and validated ok is rebuilt from the
             # candidate; a sibling's not-yet-fixed failure (in failing_nodes, untouched
             # by the fix) is left out and cloned; a node promoted since the rejection
             # matches current_prod and is never a seed. Falls back to the
             # current_prod-only diff when the verified release is unreadable or has
             # no candidate topology.
