...
| `GET /releases` | Paginated release history, newest-first. Query params: `status` (optional exact-match filter), `limit` (default 20; values that are unparseable, non-positive, or exceed 100 fall back to the default of 20), `cursor` (opaque keyset cursor). Response: `{"releases":[{release_id, status, created_at, resolved_at, node_count, bootstrap, shadow, reject_reason, repo, commit_sha}], "next_cursor":"<opaque or empty>"}`. `resolved_at` is non-empty once a release reaches any terminal status, including `validated`. `repo` and `commit_sha` are the release's stored provenance, carried on each row so the UI can resolve the commit author for the Releases tab without a per-release detail fetch. |
...
**A shadow verifies the rejected release's candidate, not production.** A shadow is a single-service delta like every other release: its own service carries the fix, and every OTHER service is assembled from the live `service_prod` pointers. That is wrong whenever the fix lands in a different service than the one whose release was rejected — a downstream service edited to repair an upstream rejection — because the rejected release's own service would then contribute its PRODUCTION manifest, and the fix would be judged on a graph the failure never occurred in. The submission therefore carries `verifies_release_id` (accepted only together with `shadow`; `ReceiveCandidateInput.validate` rejects it otherwise), stored immutably on the release row (`releases.verifies_release_id`, `V20__releases_verifies_release_id.sql`, insert-only). Every manifest-set assembly goes through `assembleFor`, which, for a shadow naming a verified release, reads that release and swaps ITS changed service to the candidate manifest key `CanonicalManifestKey(bucket, service, verified_release_id, kind)` and to that release's own image tag for the service. Both assembly sites use it — queue advance (which is also where a python shadow emits `release.requested:v1`) and the compile result (where a dbt release's manifest keys are emitted) — so a shadow's two legs cannot disagree about which manifest a service contributes. A verified release that cannot be read, or that never parsed and so has no candidate manifest, is logged and the set is left as assembled: the shadow still runs, against production, which is a weaker verification rather than a stuck one. **Only the rejected release's own delta is restored**: sibling edits the same attempt made in OTHER services are not co-verified, because one release is one service delta. Each shadow sees its own change plus the rejected candidate, and the follow-up release that runs once the fix PR is merged remains the gate that judges the whole change together.

**A shadow measures only the delta its fix adds over both production and the rejected candidate.** Assembling the rejected release's candidate for the sibling service (above) is half the mechanism; the other half is the change-detection gate. A shadow's changed set is still derived by `content_hash` diff, and diffing the assembled topology against `current_prod` — which never advanced past the rejection — would read every not-yet-fixed failure the shadow carries unchanged as "changed" and re-validate it. A two-service fix submits one shadow per edited service (one release is one service's delta), so each shadow assembles the OTHER service's still-broken node unchanged; against production, that sibling failure would sink the shadow on a node the fix was never about, and no two-service proposal could pass. So a shadow naming a verified release re-validates a node **only if the fix changed it relative to BOTH `current_prod` AND the rejected candidate** — the intersection of the two `content_hash` diffs. The fix's own edited node differs from production (old or absent) and from the rejected candidate (broken), so it is validated. A sibling service's still-unfixed failure differs from production but is byte-identical to the rejected candidate the shadow assembled it from, so it is in only one diff and excluded — this holds whether that sibling node is new in the rejected candidate or an edit to a node already live in production (where `current_prod` holds it at its pre-rejection hash, distinct from the shadow's still-broken copy). An unrelated node another release promoted since the rejection matches `current_prod` — even though it differs from the rejected candidate's older, since-superseded hash — so it too is in only one diff and is never dragged into a fix-only shadow. A descendant of the fixed node whose own content is unchanged is not a seed but is still validated through the descendants closure, exactly as for a production release. The intersection is confined to the shadow-with-`verifies_release_id` path; a production release, and a shadow naming no verified release, diff against `current_prod` alone. When the verified release is unreadable or never produced a candidate topology, the shadow falls back to a production diff — the same graceful degradation the assembly step takes.

A shadow release is submitted for either kind, and the kind decides what it actually runs. A `python` shadow reads the packaged contract yaml `agent-remediation` uploaded under the shadow's own canonical key. A `dbt` shadow runs the real compile leg against the team's image, and the proposed source reaches it through `source_overlay_uri`: the S3 URI of a gzipped tar of project-relative files, accepted only on a shadow (`ReceiveCandidateInput.validate` rejects it otherwise), stored on the release row, and threaded verbatim onto `compile.requested:v1` at queue advance and onto `seed.build.requested:v1` when the parsed release has seeds to build. Executor-controller turns it into an `overlay` init container plus a staging prologue on every team-image init container of the compile Job, which copies the team project into a writable `/work` emptyDir, lays the proposed files over the copy, and runs dbt there (see `services/executor-controller.md`), so the manifest the release parses, the candidate SQL it rewrites, and the validation Jobs it runs all describe the proposed fix rather than the committed source. Nothing in this service reads the tarball; it is a pointer it carries. On failure a shadow release is rejected like any other and emits `release.rejected:v1` — carrying `shadow: true`, which is what lets the remediation classifier record the rejection without healing it (see `services/remediation.md`). It is also rejected, rather than passed, when its candidate diff against production is empty: `validated` means the fix was validated, so it is never reached with an empty validation set (see the change-detection gate below).
...
status=ok:
  ...
  derive changed = candidate nodes whose content_hash differs from current_prod, or are new
             # for a shadow naming a verified release: the INTERSECTION of two diffs —
             # nodes that differ from current_prod AND from that release's candidate
             # topology. A sibling's not-yet-fixed failure differs from current_prod but
             # matches the rejected candidate (excluded); a node promoted since the
             # rejection matches current_prod but differs from the stale candidate
             # (excluded); only the fix's own edit differs from both (validated). Falls
             # back to the current_prod-only diff when the verified release is unreadable
             # or has no candidate topology.
