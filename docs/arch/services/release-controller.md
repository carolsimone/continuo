...
status=ok:
  ...
  baseline = current_prod.topology_snapshot
             # for a shadow naming a verified release: current_prod OVERLAID with that
             # release's candidate topology (OverlayTopology), so a node still identical
             # to the rejected candidate — another service's not-yet-fixed failure — is
             # NOT re-derived as changed; falls back to current_prod when the verified
