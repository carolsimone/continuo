...
| `GET /healthz` | Readiness, backed by the `pkg/liveness` registry. Fails (503) when any registered worker (the stream consumers, the outbox publisher) has exited with an error, any consumer read-loop heartbeat has gone stale, **or** any dependency probe (Redis, Postgres, the `outbox_dead_letters` backlog check) fails; a dependency outage — or a non-zero dead-letter backlog — pulls the pod out of the Service endpoints without restarting it, since neither is fixed by a restart. |
