...
| `service_prod` | One row per dbt service — the live per-service production pointer: `{service_name, release_id, manifest_s3_key, image_tag, runtime_manifest_uri, runtime_manifest_sha256, runtime_manifest_dbt_version, runtime_manifest_parse_context_sha256, updated_at}`. Records which manifest key, image tag, and runtime manifest artifact are currently live for each service; the full production manifest set is reconstructed by collecting every service's pointer at activation time. The four runtime manifest columns are nullable and constrained all-or-none (`service_prod_runtime_manifest_all_or_none`): a service that publishes no artifact carries none and its nodes execute down the per-node Job path, while a partial reference — which would name an artifact that cannot be verified — is rejected at the row level. |
...
### Runtime manifest carry-forward
An incremental release changes one service, but promotes a whole topology, so each service's nodes must keep pointing at their own artifact. `manifest-controller` re-parses every manifest in the assembled set and returns `runtime_manifests` keyed by service; `attachRuntimeManifests` stamps each node with its own service's reference before validation. The changed service's nodes take the reference its new parse produced; every other service's nodes take the one its manifest still declares, which is the artifact its `service_prod` pointer already names — so an unchanged service is carried forward onto its existing artifact rather than retargeted at the promoting service's.

Attachment is fail-closed on the two ways the parse result and the topology can disagree, since either would otherwise silently mis-target a service:

- A reference that fails validation rejects the release, naming the offending service.
- A reference keyed by a service with **no node** in the topology rejects the release. Dropping the key would hide that the parse and the topology disagree about what the release contains.

A service with no entry in `runtime_manifests` keeps the zero reference and flows through: it has no artifact, and its nodes execute down the per-node Job path.

On promotion, the changed service's `service_prod` pointer is upserted with the reference its release published, so the next release for any other service assembles against it.

### Activation guard
Before a release activates, the queue advance verifies that every service live in the `current_prod` topology snapshot is covered by a `service_prod` pointer (or is this release's changed service). If a live service has no pointer, the release stays `Received`, nothing is emitted, and a warning naming the uncovered services is logged; a later queue advance retries automatically. This protects the transition from a fully-populated `current_prod` with an empty `service_prod` table: without coverage, assembly would omit the unpointered services and promotion would retire them. `service_prod` is populated from the existing `current_prod` snapshot by the `seed-service-prod` command (shipped in the service image), which records each service's existing manifest key verbatim along with the runtime manifest reference its nodes carry, and is idempotent.
