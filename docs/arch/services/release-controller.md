...
**A shadow measures only the delta its fix adds to the rejected candidate.** Assembling the rejected release's candidate for the sibling service (above) is half the mechanism; the other half is the change-detection gate. A shadow's changed set is still derived by `content_hash` diff, and diffing the assembled topology against `current_prod` — which never advanced past the rejection — would read every not-yet-fixed failure the shadow carries unchanged as "changed" and re-validate it. A two-service fix submits one shadow per edited service (one release is one service's delta), so each shadow assembles the OTHER service's still-broken node unchanged; against production, that sibling failure would sink the shadow on a node the fix was never about, and no two-service proposal could pass. So a shadow naming a verified release baselines its changed set on the rejected release's **candidate topology overlaid with `current_prod`** (`OverlayTopology`): `current_prod` wins by `unique_id`, and the rejected candidate only fills in nodes `current_prod` doesn't have — the still-unpromoted nodes this shadow inherited from that rejection, including the sibling's unfixed failure. `current_prod` winning matters because it can have moved on since the rejection: an unrelated release may have promoted a node in the meantime, and the rejected candidate would otherwise still measure it at its older, since-superseded hash — spuriously re-validating a live, already-promoted node the shadow's own assembled copy matches at its current hash. A node the shadow still holds byte-identical to the rejected candidate — the sibling's unfixed failure — matches the baseline and is not re-validated, while the fix's own edit departs from the candidate and still is — as does a descendant of the fixed node, whose upstream's hash changed. The overlay is confined to the shadow-with-`verifies_release_id` path; a production release, and a shadow naming no verified release, diff against `current_prod` alone. When the verified release is unreadable or never produced a candidate topology, the shadow falls back to a production diff — the same graceful degradation the assembly step takes.
...
status=ok:
  ...
  baseline = current_prod.topology_snapshot
             # for a shadow naming a verified release: that release's candidate topology
             # OVERLAID WITH current_prod (OverlayTopology) — current_prod wins by
             # unique_id, so a node promoted since the rejection is measured at its
             # current hash, not the rejected candidate's stale one; the rejected
             # candidate only fills in nodes current_prod doesn't have (the sibling's
             # not-yet-fixed failure), which then matches the baseline and is NOT
             # re-derived as changed; falls back to current_prod alone when the verified
             # release is unreadable or has no candidate topology
  derive changed = candidate nodes whose content_hash differs from baseline, or are new
