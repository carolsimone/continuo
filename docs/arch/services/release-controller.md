...
Releases run a FIFO queue: one release is active (`compiling`, `parsing`, `seed_building`, or `validating`) at a time; on each terminal outcome the queue advances the next queued release. The full state machine is: `received` → `compiling` → `parsing` → `seed_building` → `validating` → (`promoted` | `rejected` | `superseded`). The domain also exposes `validated` as a further terminal outcome from `validating`, reached via `TransitionToValidated`, for a `shadow` release; as of this revision no handler in the running pipeline calls it, so a `shadow` release still runs the same path as any other and is promoted on success.
