...
| Candidate manifest/contract parsing, artifact rewrite, and S3 upload | `manifest-controller` | Redis + S3 (`candidate-sql/<release_id>/candidate_<unique_id>.<sql\|json>` per node — `.sql` for a dbt model/snapshot, `.json` for a python node, skipped for a dbt seed; `code-bundles/<release_id>/bundle.json` per release) |
