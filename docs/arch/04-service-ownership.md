...
| Category | Owned / used surface |
|---|---|
| Durable state | `executor_deployments`, `executor_outbox`, `message_processing`, `cancelled_schedules`, `validation_aggregates`, `executor_worker_pools` |
| gRPC server methods owned | none |
| HTTP server routes owned | `/health`, `/ready`, plus the internal worker API on the same port: `GET /internal/v1/worker/runtime`, `POST /internal/v1/workers/{claim,initialization}`, `POST /internal/v1/leases/{id}/{start,heartbeat,result-urls,complete}`. Pull-only — worker pods call the executor, never the reverse. Authenticated per pool; under the deployed default no pool is registered, so every internal route answers `401` |
| Redis consumes | `query.model:v1`, `retry.task:v1`, `schedule.cancelled:v1`, `executor.job.terminal:v1`, `validation.requested:v1`, `validation.node.completed:v1`, `validation.result:v1` (kind=complete, for candidate schema teardown) |
...
| External DB writes | dbt warehouse (`DBT_POSTGRES_DB`) — creates the `_candidate_<release>` schema on `validation.requested:v1` via `CandidateSchemaCreator`; drops it on `validation.result:v1` kind=complete via `CandidateSchemaCleaner` |
| Outbound gRPC calls | none |
...
- **Enqueue and cancellation are serialized per schedule.** `QueryModelHandler` and `RetryTaskHandler` take a `pg_advisory_xact_lock` keyed on the schedule before reading the `cancelled_schedules` guard, and `ScheduleCancelledHandler` takes the same lock before recording the cancellation. The two paths otherwise settle a schedule through tables the other writes — the guard reads `cancelled_schedules`, the cancel scan reads `executor_deployments` — which READ COMMITTED alone does not order, and a row locking clause on the guard cannot order either, since it locks only rows that already exist. The lock makes every deployment of a cancelled schedule either visible to the cancel scan or never created; without it an enqueue whose guard has read "not cancelled" but has not yet committed would outlive the cancellation, dispatch, have its terminal absorbed, and hold its execution slot with no event left to release it. It is always taken before any row lock, and no transaction holding a row lock waits on it, so it orders consistently with the dispatcher's capacity lock.
- **A cancelled schedule's Kubernetes Job runs to completion after its slot is returned.** Cancelling a deployment releases its slot but does not delete the Job; k8s-controller's `schedule.cancelled:v1` binding is a projection sink and issues no Kubernetes call. The executor therefore dispatches fresh work against the returned slot while the cancelled Job is still running, so the number of dbt Jobs concurrently on the cluster can transiently exceed `MAX_CONCURRENT_EXECUTIONS`. The overshoot is bounded by the count of cancelled-but-still-running Jobs and drains on its own as each terminates.
- **The executor owns worker identity; a worker proves it, and is trusted for nothing else.** `executor_worker_pools` holds each pool's credential digest, and the executor authenticates every internal-API call against it in constant time. The pool a claim is served from, the command a task runs, and the object keys a task's results are written to are all read from the executor's own rows — never from a request. A worker reports observations (that its process started, that it finished, how its failure looked); it never supplies an identifier the executor acts on. The pools are registered by the executor's own reconciler for the work routed to workers; under the deployed default (`EXECUTION_MODE=jobs`, no overrides) nothing is, so the table is empty and the API authenticates nobody.
- **A worker's lease token is the fence; its pool credential is the boundary.** Every lease-bearing transition is authorized by the raw lease token, whose digest alone is stored. The token is checked first and the task's pool second, so a caller that does not hold the lease learns only that its lease is stale and cannot probe for tasks; a caller that does hold it, but authenticated as another pool, is refused. Both refusals are terminal for the report — the worker stops rather than retries.
- **A cancelled task stops its worker's pod and fences what the pod still sends.** Cancelling a task releases its slot, deletes the pod running it, and keeps the lease. The deletion is what stops the dbt process; keeping the lease is what lets a worker outliving the deletion's termination grace period still authorize, so its next heartbeat or terminal report is answered `410 cancelled` — terminal for the worker — rather than a fault it would retry against. The pod is deleted by UID, so a pod the pool has already replaced under the same name is not taken out instead, and the deletion is requested before the row is cancelled: its failure rolls the whole cancellation back rather than returning a slot while the pod still runs. The overshoot is therefore bounded by the grace period, not by the life of the pod.
...
- **Candidate schema teardown.** A dedicated consumer on `validation.result:v1` (group `executor-validation-result-teardown`) reacts to the `kind=complete` message and calls `CandidateSchemaCleaner.DropCandidateSchema` against `DBT_POSTGRES_DB`; this drops the shared `_candidate_<release>` schema regardless of pass/fail outcome.
...
| S3 reads | `GetObject` on each release's `manifest.json` and its sibling `runtime-manifest.json` descriptor (key derived from the manifest key, never listed). A missing descriptor is a manifest-only release; a descriptor that fails validation causes `status=failed` with `error_class=MalformedRuntimeManifest`; two manifests declaring the same service with different runtime manifest references cause `status=failed` with `error_class=ConflictingRuntimeManifest` |
