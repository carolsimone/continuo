...
Domain purity has its own pair of guards. `TestDomainPackagesDoNotImportStreams` (`pkg/streams/domain_imports_test.go`) walks every module's `domain/` tree and fails on an import of `pkg/streams`; `topology-controller/tests/test_domain_imports_no_streams_contract.py` does the same for `streams_contract` under `topology-controller/domain`. Stream and consumer-group names are transport. The closed value sets domain code does name — the `vocabularies:` block of `pkg/streams/contract.yaml` — are generated into `pkg/domain/model` and `topology-controller/domain/contract_vocabulary.py` instead, so a domain package spells `model.ParseFailureKind` without depending on the messaging contract.
...
| Redis consumes | `query.model:v1`, `schedule.cancelled:v1`, `validation.requested:v1`, `validation.result:v1` (kind=complete, for candidate schema teardown), `seed.build.requested:v1`, `compile.requested:v1`, `pipeline.run.finished:v1`, `release.promoted:v1`, `release.rejected:v1`, `check.k8s:v1` |
| Redis produces | `check.k8s:v1` (via the delay queue — the dispatcher writes the first ticket after Job creation, the job-status handler re-writes one on every still-running observation), `task.status.updated:v1` (RUNNING + SUCCEEDED/FAILED from the job-status handler, plus FAILED on the never-deployed terminal dispatch failure), `task.execution.recorded:v1`, `node.updated:v1`, `validation.result:v1` (unified validation leg: `kind=node` per-node projections as each node settles, then a trailing `kind=complete` per-release decision), `seed.build.completed:v1`, `compile.completed:v1` |
...
- **Inbound handlers write only to `deployments`.** `QueryModelHandler` commits a `pending` row inside its Unit-of-Work transaction; the job-status handler's retry branch commits the same kind of row, in the same transaction as the FAILED announcement it writes for the exhausted attempt. No Kubernetes I/O occurs during inbound message handling.
...
- **Topological unblock/skip on node completion.** When the job-status handler observes a validation Job's terminal status, it calls `outcomes.Recorder.Record` in the same transaction: this transitions `blocked` downstreams whose every in-set upstream is now `ok` to `pending` (ready for dispatch); on a node failure it marks transitively `blocked` downstreams `skipped` (terminal non-`ok`). `blocked` is non-terminal; `skipped` fails the release.
...
| Category | Owned / used surface |
|---|---|
| Durable state | Postgres `release_pipeline_runs` (one row per run of either kind, `run_kind` ∈ `candidate`\|`verification` — shared non-terminal statuses `received`, `compiling`, `parsing`, `seed_building`, `validating`, then a kind-specific terminal: `promoted`\|`rejected`\|`superseded` for a candidate, `passed`\|`failed` for a verification — `manifest_kind` (`dbt`/`python`), `changed_service`, assembled per-service `image_tags`, candidate topology including per-node `candidate_artifact_uri`, the run-level `code_bundle_uri` set from the parse result, validation results, transitions; candidate-only `repo` + `commit_sha` + `bootstrap` + `remediation_round` + `rejection_payload`; verification-only `verifies_release_id` + `attempt` + `source_overlay_uri`), `current_prod` (singleton live `topology_snapshot` + promoted `release_id`; `candidate_artifact_uri` is stripped on promotion), `service_prod` (one row per service, dbt or python: live `manifest_s3_key` + `manifest_kind` + `image_tag` + `release_id`), `message_processing`, `release_controller_outbox` |
| HTTP server | `POST /releases` (single-service candidate; requires `repo` + `commit_sha`; optional `kind` (`dbt`/`python`, default `dbt`) and `bootstrap`; `source_overlay_uri`/`verifies_release_id` refused, 400 — those belong to `POST /verification-runs`), `GET /releases` (candidates only), `GET /releases/{id}` (candidates only; returns `repo` + `commit_sha`), `POST /verification-runs`, `GET /verification-runs/{id}`, `GET /verification-runs?verifies=`, `GET /pipeline`, `GET /current-prod`, `GET /healthz` |
| gRPC server methods owned | none |
| Redis consumes | `compile.completed:v1`, `manifest.loaded.candidate:v1`, `seed.build.completed:v1`, `validation.result:v1` |
| Redis produces | `release.requested:v1`, `validation.requested:v1` (per node: `candidate_artifact_uri`), `release.promoted:v1` (top-level `code_bundle_uri`, `bootstrap`; candidate-only), `release.rejected:v1` (candidate-only — a verification run's failure never rides this stream; on `validation_failed` also includes top-level `repo` + `commit_sha` and, per node, `candidate_artifact_uri` plus the candidate topology's `node_type`, `file_path`, and `service`), `pipeline.run.finished:v1` (either kind, every terminal outcome: `run_id`, `run_kind`, `outcome`, `service`, `candidate_schema`, `verifies_release_id`, `attempt`) |
...
- **One service per release.** `POST /releases` accepts `{service, release_id, image_tag, repo, commit_sha, kind?, bootstrap?}` — a delta for a single service, dbt or python. `repo` (GitHub owner/name) and `commit_sha` (full SHA) are required provenance fields captured at receipt, stored as NOT NULL immutable columns, and returned on `GET /releases/{id}`. The full per-service manifest set and image-tag map are assembled later, at activation, never at receipt.
- **A fix-verification run never promotes.** A run posted to `POST /verification-runs` carries a fix proposed by agent-remediation that no human has approved. It runs the same parse -> candidate-schema -> validation pipeline as a candidate release and then stops at a terminal `passed`/`failed` verdict, so it never writes `current_prod`, never upserts `service_prod`, and never emits `release.promoted:v1` or `release.rejected:v1`. The `pipeline.Run` aggregate enforces both halves: `Promote` refuses a non-candidate, and `Pass` refuses a non-verification, so neither kind can reach the other's terminal status.
