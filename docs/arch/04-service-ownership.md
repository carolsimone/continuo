...
| Redis consumes | `query.model:v1`, `retry.task:v1`, `schedule.cancelled:v1`, `validation.requested:v1`, `validation.result:v1` (kind=complete, for candidate schema teardown), `seed.build.requested:v1`, `compile.requested:v1`, `pipeline.run.finished:v1`, `release.promoted:v1`, `release.rejected:v1`, `node.deployed:v1`, `check.k8s:v1` |
| Redis produces | `node.deployed:v1`, `check.k8s:v1` (via the delay queue), `task.status.updated:v1` (RUNNING + SUCCEEDED/FAILED from the job-status handler, plus FAILED on the never-deployed terminal dispatch failure), `task.execution.recorded:v1`, `node.updated:v1`, `retry.task:v1`, `task.failed:v1`, `validation.result:v1` (unified validation leg: `kind=node` per-node projections as each node settles, then a trailing `kind=complete` per-release decision), `seed.build.completed:v1`, `compile.completed:v1` |
