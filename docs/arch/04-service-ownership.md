...
- **A cancelled task reaches its worker only at the next heartbeat.** Cancelling a task releases its slot but neither stops the pod nor drops the lease, so the worker keeps running dbt until its heartbeat is answered `410 cancelled`, which is its only notice. This is the worker-path counterpart of a cancelled schedule's Kubernetes Job running to completion, and bounds the same overshoot.
