...
| Validation | A declared `output_columns` entry missing from the CSV's own header line | Validation failure → `release.rejected:v1` (`stage=validation`) → the `remediation` classifier → `remediation.requested:v1` → `csvValidationFixer` (see `docs/arch/services/agent-remediation.md`). |
