...
**Stage sources.** The classifier receives each failure annotated with a `source` field (`validation`, `compile`, `seed_build`, or `duplicate_table`) derived from the `stage` field of the incoming `release.rejected:v1` event; the stage-less `duplicate_table` rejection instead derives its source from the `reason` field. For `compile` and `seed_build` sources the classifier additionally calls `ExtractDbtFilePath` against the dbt log to derive the project-relative source file path (e.g. `models/order_items.sql`); this `file_path` is threaded into the `remediation.requested:v1` trigger so the downstream agent can read the real source file without an orchestrator round trip. For `validation` sources `file_path` is empty at this layer; the agent resolves it via the orchestrator's `GetNodeLocation` RPC.
...
A `python-csv` node's contract declares a single `csv` read (an `s3://` or `https://` uri) and the `output_columns` it promises. Its failures fall into five distinct classes, at three different points in the pipeline — most never reach the `remediation` classifier above at all, since only the validation-stage class does:

| When | Failure | Classification |
|---|---|---|
| CI / manifest-controller parse | A malformed `csv:` uri (not `s3://` or `https://`), or a `reads` block that is not exactly `{csv: <uri>}` | Permanent contract error — the release never reaches validation; `manifest-controller`'s parser rejects the entry outright (see `docs/arch/services/manifest-controller.md`). |
| Validation | A declared `output_columns` entry missing from the CSV's own header line | Validation failure → `release.rejected:v1` (`stage=validation`) → the `remediation` classifier → `remediation.requested:v1` → `csvValidationFixer` (see `docs/arch/services/remediation-agent.md`). |
| Validation | The declared uri is unreachable (network error, 404, access denied) when the validation pod range-fetches its header | Validation failure, same routing as above — an accepted trade-off: an unreachable source at validation time surfaces as an ordinary validation rejection rather than a distinct infra category, since a bad or since-moved uri is functionally a contract defect from the operator's perspective. |
