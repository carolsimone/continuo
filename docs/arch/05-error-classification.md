...
|---|---|---|
| `execution-controller/adapters/k8s/jobs_create.go:741` (`buildPodSpec`) | `image_tag` missing on dispatch | `fmt.Errorf("%w: image_tag missing from job params for service %s", events.ErrPermanent, params.ServiceName)` |
...
| `orchestrator`, `execution-controller` outbox publishers (`adapters/*/outbox_publisher.go`, `Publish`) | decode a typed event out of `entry.Payload` per `event_type`; a `json.Unmarshal` failure against a *known* `event_type`, or an out-of-range numeric field (`pkg/num.Int32`) on an otherwise well-formed known payload, is deterministic — the row can never succeed no matter how many times it is retried | `fmt.Errorf("%w: unmarshal <event>: %v", events.ErrPermanent, err)` / `fmt.Errorf("%w: <event> payload: %v", events.ErrPermanent, err)` |
| `orchestrator`, `execution-controller` outbox publishers (`adapters/*/outbox_publisher.go`, `Publish`) | the type-switch's `default:` case — an `event_type` the publisher does not recognise — is **not** wrapped in `ErrPermanent`. During a rolling deployment an old replica can dequeue a row for an `event_type` only a newer (already-deployed) replica knows how to publish; treating it as permanent would dead-letter a row that a peer replica can handle moments later. The row is retried through its normal backoff budget instead | `fmt.Errorf("<service> publisher: unknown event_type %q (retryable — a newer replica may handle it during a rolling upgrade)", entry.EventType)` |
| All 6 outbox publishers' `Publish`, dead-letter branch | `pkgoutbox.DeadLetterValues` fails to decode a dead-letter row's own JSON payload — the publisher's own construction, so a decode failure is a bug, not an infra blip | `fmt.Errorf("%w: dead-letter values: %v", events.ErrPermanent, err)` |
...
| `execution-controller/service/deployer/dispatcher.go` (`dispatchOne`) | call `writeFailedAnnouncements` (writes `task.status.updated:v1` FAILED + `node.updated:v1` FAILED as `execution_outbox` rows, marks `deployments` row `failed`) |
...
`execution-controller`, `release-controller`, `remediation`,
`agent-remediation`.
...
The `remediation` service applies a separate, domain-level classification to failed nodes across all four pipeline stages: `parse`, `compile`, `seed_build`, and `validation`. This is distinct from the `ErrPermanent` sentinel above, which governs message-consumer routing. The same four categories cover dbt and python nodes alike — a python node's validation failure arrives as the warehouse engine's own error text, which the substring rules below read exactly as they read a dbt log's. The classifier (`remediation/domain/failure/classify.go`) deterministically sorts each failure into one of four categories regardless of which stage or node kind produced it — except a `stage=parse` failure, which never runs these substring rules at all (see **Parse-stage classification** below):
...
| `test` | `emit` | dbt test assertion failures — an actual `dbt test` run reporting `fail` on the `test` operation. A `dbt-test` node's validation-stage bind-check failure is a different thing and never lands here: `check_binds` EXPLAINs the test's compiled assertion query rather than running it, so it can only report an EXPLAIN error, never a test-assertion `fail`. That error line is classified by the ordinary `logic`/`infra_transient`/`unknown` rules elsewhere in this table exactly like any other dbt validation failure — e.g. `logic` for a missing column or relation the release dropped. |
...
**Parse-stage classification.** `stage=parse` evidence is classified from its contract kind by `ClassifyParse`, with no log fetch and no text rules. The kind is a `parse_failure_kind` value — the vocabulary declared in `pkg/streams/contract.yaml` and generated into `pkg/domain/model` as `model.ParseFailureKind`, which is what the classifier's `IsValid()`/`Healable()` calls read, so the classifier's healable set is the contract's: `invalid_sql` and `unqualified_reference` are `logic` and emitted (reason `logic:<kind>`, signature keyed on kind and node id, excerpt = the parser's detail); any other kind is dropped with reason `parse:not_healable`. topology-controller only ever publishes a non-empty `failed_nodes[]` for the two healable kinds — `invalid_artifact` and `internal_error` fail the release on the first manifest or upload problem found instead of collecting one — so release-controller's `stage=parse` per-node entries are non-empty only for those two kinds, and the drop path covers a producer newer than this build. Node kind plays no part here: a python node's parse failure is classified and emitted exactly as a dbt node's is. The two diverge only downstream, in `agent-remediation`, where `node_type` on the trigger selects the lane: a `python-model` node's rejected SQL is one of its `reads` in its service's `contract.yaml`, not the script `file_path` names, so it takes the python contract-fix lane (the yaml is located from the node id, the read's SQL corrected, and the repair proven by a python verification run), while a `python-csv` node — whose only read is an S3 URI the parser has nothing to reject in — is recorded `skipped` on the attempt with that reason (see `docs/arch/services/agent-remediation.md`, **Parse fixer**).

**Stage sources.** The classifier receives each failure annotated with a `source` field (`parse`, `validation`, `compile`, `seed_build`, or `duplicate_table`) derived from the `stage` field of the incoming `release.rejected:v1` event; the stage-less `duplicate_table` rejection instead derives its source from the `reason` field. For `compile` sources the classifier additionally calls `ExtractDbtFilePath` against the dbt log to derive the project-relative source file path (e.g. `models/order_items.sql`); for `seed_build`, `validation`, `duplicate_table`, and `parse` sources release-controller stamps `file_path`/`service` onto each `per_node` entry directly instead — from the candidate topology for `seed_build`/`validation`/`duplicate_table`, and from topology-controller's own `failed_nodes` report for `parse`. Either way the path is threaded into the `remediation.requested:v2` trigger so the downstream agent can read the real source file without an orchestrator round trip — which matters for correctness, not only economy: `GetNodeLocation` serves the *promoted* topology, and a rejected release is never promoted. Only a node whose trigger carries no `file_path` falls back to that RPC.
...
`release.rejected:v1` is candidate-only: a fix-verification run's failure never rides this stream, so the classifier never sees one and needs no override for it. Agent-remediation's own verification reconciler learns a run's verdict by polling the run it submitted, through `pipeline.run.finished:v1` and `GET /verification-runs/{id}` — a path this classifier is not part of.

**Structured-first signal.** Each per-node entry on `release.rejected:v1` carries an optional `run_results_uri` — the S3 key of a structured validation-result record (`status` in dbt's `success | error | fail | skipped` vocabulary, plus `message`, `failures`, `unique_id`) emitted by the validation pod and uploaded by the job-status handler. When present, `ClassifyWithStructured` (`remediation/domain/failure/classify.go`) keys off it: a `fail` status is deterministically `test` (no heuristic); an `error` status routes the structured `message` through the same infra/logic substring rules used for the text log. A `check_binds` op — the validation-stage bind check a `dbt-test` node runs — never reports `fail`, since it EXPLAINs the test's query rather than executing its assertion; a binding failure always reports `error` and so always takes the substring-rule path, never the deterministic `test` category. When `run_results_uri` is absent, empty, or unfetchable — or the structured record carries no message — classification falls back to parsing the dbt text log at `dbt_log_uri`. The category vocabulary and routing decisions above are identical on either path.
...
- `docs/arch/04-service-ownership.md` (per-service invariants under
  orchestrator, execution-controller, and topology-controller).
