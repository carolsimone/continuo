...
### 11a. Batched Remediation and Fix-Verification Runs

One rejected release is one unit of remediation, whichever of the five sources rejected it — `parse`, `compile`, `seed_build`, `validation`, or `duplicate_table`. However many nodes it failed on, the classifier emits **one** trigger, the agent produces **one** attempt, and a human reviews **one** pull request. A parse rejection carries the parser's detail inline on each `per_node` entry, so the classifier reads no log and the agent shows that detail in place of one. And no fix is offered for review until a real run has proven it: every attempt ends by submitting a **fix-verification run** per edited service to `release-controller` — a run of `run_kind="verification"` that takes the identical pipeline legs a candidate release takes (compile → parse → seed_build → validate, sharing the same FIFO queue) but ends at the verification terminal `passed` or `failed` rather than the candidate terminal `promoted` or `rejected`, and never reads or writes `current_prod`.
...
sequenceDiagram
  ...
  loop one fix-verification run per edited service
    Note over RA: run_id = VerificationRunID(release_id, service, attempt)<br/>= verify-<release>-<service>-a<n>
    alt python service
      RA->>S3: PutObject <service>/<run_id>/contract.yaml (packaged contract)
    else dbt service
      RA->>S3: PutObject <service>/<run_id>/source-overlay.tar.gz<br/>(proposed files, keyed relative to the project root)
    end
    RA->>RC: GET /releases/{original_id} (image tag for the service)
    RA->>RC: POST /verification-runs { run_id, service, image_tag, kind,<br/>verifies_release_id=original_id, attempt, source_overlay_uri (dbt only) }
    RC-->>RA: 202 { run_id, status: "received" }
  end
  Note over RA: ONE proposal(status=verifying) — resolved_node_ids, node_outcomes,<br/>verifications[] (one {service, kind, run_id, phase, activated_at, error} per run),<br/>file_edits[], and the raw trigger for a possible retry

  Note over RC: each fix-verification run takes its own turn on the shared FIFO<br/>queue and runs the same legs a candidate release runs (Flow 11): compile<br/>(dbt only) → parse → seed_build (if needed) → validate. A dbt run's compile<br/>Job lays the overlay over the team project first. It never reads or writes<br/>current_prod or service_prod, and ends only passed or failed.
  Note over RC: at the run's terminal — of either kind — the same transaction<br/>that sets its status writes pipeline.run.finished:v1 {run_id,<br/>run_kind="verification", outcome, service, candidate_schema,<br/>verifies_release_id, attempt, finished_at}
  RC->>R: publish pipeline.run.finished:v1
  R->>EC: consume pipeline.run.finished:v1 (group executor-pipeline-run-finished)
  Note over EC: drop the named candidate_schema whatever the outcome<br/>(a schema the validation-result teardown already dropped is a no-op)

  loop verification reconciler (service/verification), every VERIFICATION_POLL_INTERVAL
    RA->>RC: GET /verification-runs/{run_id} — for EVERY run on the proposal
    RC-->>RA: { status, activated_at, finished_at, per_node_results, ... }
    Note over RA: map status → phase — received: queued;<br/>compiling/parsing/seed_building/validating: running;<br/>passed: passed (terminal); failed: failed (terminal)<br/>record the phase onto verifications[] whenever it changed
  end

  alt every run's phase is passed
...
sequenceDiagram
  ...
  alt not found
    ...
    RA-->>UI: { pr_services: [...] } — [""] for an unsplit proposal
    ...
    loop each service in services, independently
      ...
      alt already opening or open for this service
        ...
        alt edits list empty and no single-file fallback
          ...
          Note over UI: record a per-service error, continue to the next service
        else edits usable (an edits-empty row falls back to its single-file fields)
...
sequenceDiagram
  ...
  loop every REMEDIATION_PR_POLL_INTERVAL (default 60s)
    ...
    loop each stuck-opening (proposal, service) child row
      Note over RA: recompute branch_name = BuildBranch(release_id, attempt, service) --<br/>remediation/<release_id>/attempt<n>/<service>, or remediation/<release_id>/attempt<n> for the unsplit ("") group
