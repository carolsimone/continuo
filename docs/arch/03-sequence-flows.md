...
sequenceDiagram
  ...
  loop dispatch pending rows, unblocking downstream as upstreams settle ok
    Note over EC: build_from_sql: init fetch sidecar (s3-sidecar) downloads CANDIDATE_SQL_URI → CANDIDATE_SQL_PATH on shared emptyDir;<br/>main dbt-job (dbt-base, no S3): validation_runner.py reads CANDIDATE_SQL_PATH → CREATE TABLE {candidate}.{table} AS (SQL) WITH NO DATA<br/>clone_from_prod: single dbt-job container (dbt-base), no S3, no emptyDir<br/>seed: single container → dbt seed --select {table} --empty
...
**Self-contained validation (zero model edits).** The validation build set is the changed nodes, their downstream descendants, and their *full transitive upstream closure across service boundaries*. `manifest-controller` rewrites each node's compiled SQL to the candidate schema (via sqlglot), uploads it to S3 at `candidate-sql/<release_id>/<unique_id>.sql`, and emits a per-node `candidate_sql_uri` — an `s3://` reference to that object. `executor-controller` builds every upstream as an empty table (`CREATE TABLE … WITH NO DATA`, seeds via `dbt seed --empty`) in `_candidate_<release>` in dependency order, then the changed node against them. For `build_from_sql` nodes (models and snapshots that carry candidate SQL), the validation pod uses the adapter-sidecar pattern: an init `fetch` container (the shared `s3-sidecar` image, which carries no dbt) downloads the SQL object from S3 at `CANDIDATE_SQL_URI` and writes it to `CANDIDATE_SQL_PATH` on a shared `emptyDir`; the main `dbt-job` container (`dbt-base`, which carries no boto3) reads the SQL from that local file and materializes the empty table. `clone_from_prod` nodes run as a single-container pod (no S3, no emptyDir) and clone their schema directly from production. Because the SQL's refs already point at the candidate schema, a model whose source still reads `FROM analytics.table_a` validates against the candidate copy — teams never template their schema names. Nothing in production is touched during validation, and the candidate schema is dropped after the aggregate result is consumed.
