...
sequenceDiagram
  ...
  alt DuplicateClaims(topology) non-empty
    ...
    RC->>R: publish release.rejected:v1
  else shadow:true AND candidate topology empty (in-set empty)
    Note over RC: a shadow release measures a proposed fix by running it through the pipeline;<br/>a candidate with no node built and checked nothing, so it must not reach the<br/>terminal "validated" a reviewer reads as proof<br/>Reject(reason=nothing_to_validate, stage-less, no per_node)<br/>the remediation classifier derives no evidence from it (no per_node), and the<br/>shadow-verify reconciler reads the rejected status as a failed fix attempt
    RC->>R: publish release.rejected:v1 {release_id, reason, error_detail, shadow:true}
  else shadow:true AND nothing to validate (in-set empty, candidate non-empty)
    Note over RC: every candidate node matches current_prod, so the candidate is production's<br/>own validated code and the fix is proven by identity with that baseline — the shape<br/>of a fix that restores a compile-broken model to its promoted content, already<br/>proved by compiling<br/>transition Validating then Validated; current_prod untouched, no event
