...
sequenceDiagram
  ...
  alt every stored node ok and aggregate_status=ok
    ...
    Note over RC: Reject(validation_failed) using stored per-node results<br/>emit release.rejected:v1 {release_id, stage="validation", reason, failing_nodes,<br/>per_node[{node_id,status,dbt_log_uri,run_results_uri,candidate_artifact_uri}], repo, commit_sha}
    RC->>R: publish release.rejected:v1
    R->>RM: consume release.rejected:v1
