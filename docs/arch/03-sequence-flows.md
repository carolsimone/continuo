...
sequenceDiagram
  ...
  R->>ST: consume release.promoted:v1 (state-release-promoted-seeds group)
  Note over ST: PromotedSeedsHandler — filter topology to Changed && node_type=dbt-seed<br/>no changed seeds ⇒ no run, no event<br/>else create a RUNNING run, kind=promote_seed, id derived from release_id<br/>(deterministic, so a redelivered promotion maps to the same run)
