...
sequenceDiagram
  ...
  Note over MC: parse model/seed/snapshot nodes<br/>content_hash = sha256(source_hash|shared_code_hash|config_hash) fold<br/>(source_hash: dbt checksum w/ sha256 fallback; shared_code_hash: transitive macro checksums, "" if none;<br/>config_hash: resolved config minus meta/docs/description/grants/tags)<br/>resolve upstreams via sqlglot (qualified refs only)<br/>rewrite SQL to _candidate_{release} schema refs via sqlglot<br/>upload rewritten SQL → S3 candidate-sql/{release_id}/candidate_{unique_id}.sql<br/>(upload failure is fatal; seeds → empty candidate_sql_uri)<br/>build one code-bundle document (contract_version 1) → S3 code-bundles/{release_id}/bundle.json<br/>(upload failure is fatal, error_class=CodeBundleUploadFailed; runs only after every node's SQL has uploaded)
  alt manifest malformed / unqualified table ref / candidate-SQL or code-bundle upload failure
    ...
    MC->>R: publish manifest.loaded.candidate:v1 {status=ok, topology[] (per node: candidate_sql_uri), code_bundle_uri}
  ...
  loop dispatch pending rows, unblocking downstream as upstreams settle ok
    Note over EC: build_from_sql: single validation container fetches CANDIDATE_SQL_URI from S3 itself → CREATE TABLE {candidate}.{table} AS (SQL) WITH NO DATA<br/>clone_from_prod: single validation container, no S3 → clone prod table shape empty<br/>(seeds and unchanged upstreams use clone_from_prod)
