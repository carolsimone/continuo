...
// node's terminal result, emitted by the job-status handler when a seed-build
// Job ends. It mirrors ValidationNodeCompleted field-for-field (the two legs
// report a per-node terminal outcome identically). The executor attaches the
// outcome to the matching (ReleaseID, NodeID) seed-build deployment, then runs
// the per-release seed-build aggregate-emit gate.
//
