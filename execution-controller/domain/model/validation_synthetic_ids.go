...
// the check_delayed ticket keyed off the same IDs.
...
// these synthetic IDs satisfy the columns and key the first check_delayed
// ticket the dispatcher writes so the job-status handler status-checks the
// validation Job. The NUL separator keeps the two business keys unambiguous
// across boundaries.
