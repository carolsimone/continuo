...
func TestNodeSource_ReturnsNodeEntry(t *testing.T) {
	...
	src, err := r.NodeSource(context.Background(), testBundleURI, "analytics.orders")
...
func TestNodeSource_EmptyURI_ErrNotFound(t *testing.T) {
	...
	r := newTestReader(srv.URL)
	_, err := r.NodeSource(context.Background(), "", "analytics.orders")
...
func TestNodeSource_NodeAbsent_ErrNotFound(t *testing.T) {
	...
	r := newTestReader(srv.URL)
	_, err := r.NodeSource(context.Background(), testBundleURI, "analytics.missing")
	require.ErrorIs(t, err, ports.ErrNotFound)
}
...
func TestNodeSource_MalformedBundle_ErrNotFound(t *testing.T) {
	...
	r := newTestReader(srv.URL)
	_, err := r.NodeSource(context.Background(), testBundleURI, "analytics.orders")
	require.ErrorIs(t, err, ports.ErrNotFound)
}
