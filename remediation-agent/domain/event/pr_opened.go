...
// PROpenedEventID derives a stable id from (releaseID, nodeID, attempt) so a
// re-emission of the same PR-opened fact dedups to one downstream event.
func PROpenedEventID(releaseID, nodeID string, attempt int) uuid.UUID {
	return uuid.NewSHA1(prOpenedNamespace, []byte(releaseID+"|"+nodeID+"|"+itoa(attempt)))
}
