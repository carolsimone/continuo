...
func TestPropose_MissCallsProviderThenPuts(t *testing.T) {
	inner := &fakeProvider{result: sampleResult()}
	cache := newFakeCache()
	p := New(inner, cache, "model-x", testLogger())
...
func TestPropose_HitSkipsProvider(t *testing.T) {
	inner := &fakeProvider{result: ports.ProposeResult{ProposedContent: "FROM PROVIDER"}}
	cache := newFakeCache()
	p := New(inner, cache, "model-x", testLogger())
	...
	if inner.calls != 0 {
		t.Fatalf("provider calls = %d, want 0 on cache hit", inner.calls)
	}
