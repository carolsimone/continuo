...
func (uow *PostgresUnitOfWork) OutboxRepo() postgres.OutboxRepository {
	// If a transaction is active, use it to ensure transactional outbox pattern
	// Otherwise use the main DB connection for non-transactional operations
	if uow.inTx && uow.tx != nil {
		return postgres.NewOutboxRepository(uow.tx, uow.logger)
	}
