r"""Tier-4 image smoke tests: the built slim postgres image fails structurally.
...
Select the tag under test with VALIDATION_IMAGE_UNDER_TEST.
"""
import os
import subprocess
...
def test_discovery_and_required_env_produce_structured_block():
    """The baked postgres adapter is discovered; its missing POSTGRES_* env fails cleanly."""
    proc = _run({
    ...
    # Discovery loaded exactly one adapter (postgres); its required env is absent,
    # so the runner emits a structured block naming the missing vars and exits 2.
    assert proc.returncode == 2
