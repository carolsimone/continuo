...
// wantWarehouseEnvFrom is the envFrom source every dbt-running container must
// carry when VALIDATION_WAREHOUSE_SECRET=wh-secret (set via t.Setenv).
func wantWarehouseEnvFrom() []corev1.EnvFromSource {
...
func TestBuildPodSpec_HydratesProdParseCache(t *testing.T) {
	...
	// Team container inline env is exactly the JobParams-derived vars; the
	// warehouse connection arrives via the warehouse-Secret envFrom.
	...
	assert.Equal(t, wantWarehouseEnvFrom(), spec.Containers[0].EnvFrom,
		"dbt container attaches the warehouse Secret via envFrom")
}
...
func TestBuildSeedBuildPodSpec_HydratesCandidateParseCache(t *testing.T) {
	...
	wantCommand := append([]string(nil), command...)
	// Team container inline env is exactly the ValidationJobParams-derived vars
	// plus DBT_TARGET_SCHEMA; the warehouse connection arrives via the
	// warehouse-Secret envFrom.
	wantEnv := []corev1.EnvVar{
...
// TestWarehouseSecretEnvFromDriftGuard pins the parity invariant: the compile
// rehearsal containers (compile + parse-prod/parse-candidate) and the run/seed
// pods attach the SAME warehouse-Secret envFrom source, and none of them
// carries any inline warehouse env. If any builder diverges, a hydrated
// partial-parse cache silently stops matching the run pod's actual connection
// and this test catches it.
func TestWarehouseSecretEnvFromDriftGuard(t *testing.T) {
	t.Setenv("VALIDATION_WAREHOUSE_SECRET", "wh-secret")
	want, err := warehouseSecretEnvFrom("test")
	require.NoError(t, err)
	require.Equal(t, wantWarehouseEnvFrom(), want)
	...
	containers := map[string]corev1.Container{
		"run team container":  runSpec.Containers[0],
		"seed team container": seedSpec.Containers[0],
	}
	for _, c := range compileSpec.InitContainers {
		containers["compile initContainer "+c.Name] = c
	}
	require.Len(t, containers, 5)
	for label, ctr := range containers {
		assert.Equal(t, want, ctr.EnvFrom, "%s: warehouse-Secret envFrom drift", label)
		for _, e := range ctr.Env {
			assert.False(t, strings.HasPrefix(e.Name, "DBT_POSTGRES"),
				"%s: leftover fabricated warehouse env %s", label, e.Name)
			assert.False(t, strings.HasPrefix(e.Name, "POSTGRES_"),
				"%s: inline warehouse env %s must come from the Secret, not env", label, e.Name)
		}
	}
}

// TestBuildPodSpecFailsPermanentWithoutWarehouseSecret pins that a dbt Job is
// failed permanently, with an actionable reason, rather than launched with no
// warehouse connection at all.
func TestBuildPodSpecFailsPermanentWithoutWarehouseSecret(t *testing.T) {
	t.Setenv("VALIDATION_WAREHOUSE_SECRET", "")
	_, err := buildPodSpec(JobParams{
		ServiceName: "service-1",
		ImageTag:    "abc123",
		NodeType:    pkg_model.NodeTypeDbtModel,
		TableName:   "orders",
	...
	require.Contains(t, err.Error(), "VALIDATION_WAREHOUSE_SECRET")
}
