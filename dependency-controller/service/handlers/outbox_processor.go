...
func (p *OutboxProcessor) processEntry(ctx context.Context, entry *model.OutboxEntry) error {
	...
	if exists {
		p.logger.Info("Outbox entry already published, marking as published",
			"entry_id", entry.ID,
		)
		// Just mark as published, skip actual publishing
	...
	// Step 3: Unmarshal payload to get event data
	var evt event.NodeReadyForExecution
	if err := json.Unmarshal(entry.Payload, &evt); err != nil {
		return fmt.Errorf("failed to unmarshal payload: %w", err)
	}

	// Step 4: Publish to Redis stream (include outbox_entry_id for downstream deduplication)
	values := map[string]interface{}{
		"outbox_entry_id": entry.ID.String(), // NEW: for executor-controller deduplication
	...
	publishedMsg := &model.PublishedMessage{
