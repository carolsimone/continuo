...
type MessageProcessingRepository interface {
	InsertIfNotExists(ctx context.Context, msgProc *model.MessageProcessing) (uuid.UUID, bool, error)
	GetByMessageID(ctx context.Context, messageID string) (*model.MessageProcessing, error)
	UpdateState(ctx context.Context, id uuid.UUID, state model.MessageProcessingState) error
}
...
// Returns (id, inserted, error) where inserted=true if new record was created
...
func (r *messageProcessingRepository) GetByMessageID(
	ctx context.Context,
	messageID string,
) (*model.MessageProcessing, error) {
	var msgProc model.MessageProcessing
...
func (r *messageProcessingRepository) UpdateState(
	ctx context.Context,
	id uuid.UUID,
	state model.MessageProcessingState,
) error {
	query := `
